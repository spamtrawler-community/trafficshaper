...
class Firewall_ProjectHoneypot_Model_Manage
{
    private $dbTableName = 'projecthoneypot_urls';
    public $oInput = NULL;
    ...
    public static function validateSettings($aSettings){
        ...
        if(!ctype_digit($aSettings['conf_status'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'ProjectHoneypot Status Invalid!';
        }

        if(!ctype_digit($aSettings['conf_params']['allowcaptcha'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'ProjectHoneypot Allow Captcha Invalid!';
        }

        /*
        if(!ctype_digit($aSettings['conf_order'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'ProjectHoneypot Order Invalid!';
        }
        */
        ...
        if(!ctype_alnum($sBlockReason)){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Project Honeypot Block Reason Invalid!';
        }

        if(!empty($aSettings['conf_params']['api_key']) && !ctype_alnum($aSettings['conf_params']['api_key'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Project Honeypot API Key Invalid!';
        }

        if(!ctype_digit($aSettings['conf_params']['last_activity'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Project Honeypot Last Activity Invalid!';
        }

        if(!ctype_digit($aSettings['conf_params']['threat_score'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Project Honeypot Threat Score Invalid!';
        }

        if(isset($aSettings['conf_params']['block_types']) && !array_filter($aSettings['conf_params']['block_types'], 'ctype_digit')) {
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Project Honeypot Blocked Visitor Types Invalid!';
        }
