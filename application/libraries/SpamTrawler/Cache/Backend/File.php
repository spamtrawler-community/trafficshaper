 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Cache_Backend_File extends SpamTrawler_Cache_Backend implements SpamTrawler_Cache_Backend_ExtendedInterface
{
    /**
     * Available options
     *
    ...
    protected $_options = array(
        ...
        'file_name_prefix' => 'SpamTrawler_Cache',
    ...
    public function __construct(array $options = array())
    {
        ...
        if (isset($this->_options['file_name_prefix'])) { // particular case for this option
            if (!preg_match('~^[a-zA-Z0-9_]+$~D', $this->_options['file_name_prefix'])) {
                SpamTrawler_Cache::throwException('Invalid file_name_prefix : must use only [a-zA-Z0-9_]');
            }
        }
        if ($this->_options['metadatas_array_max_size'] < 10) {
            SpamTrawler_Cache::throwException('Invalid metadatas_array_max_size, must be > 10');
        }
    ...
    {
        if (!is_dir($value)) {
            SpamTrawler_Cache::throwException(sprintf('cache_dir "%s" must be a directory', $value));
        }
        if (!is_writable($value)) {
            SpamTrawler_Cache::throwException(sprintf('cache_dir "%s" is not writable', $value));
        }
    ...
    public function load($id, $doNotTestCacheValidity = false)
    {
        ...
        if ($this->_options['read_control']) {
            ...
            if ($hashData != $hashControl) {
                ...
                $this->_log('SpamTrawler_Cache_Backend_File::load() / read_control : stored hash and computed hash do not match');
                $this->remove($id);
                return false;
            }
        }
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        if (!$res) {
            $this->_log('SpamTrawler_Cache_Backend_File::save() / error on saving metadata');
            return false;
        }
    ...
    public function getFillingPercentage()
    {
        ...
        if ($total == 0) {
            SpamTrawler_Cache::throwException('can\'t get disk_total_space');
        } else {
            if ($free >= $total) {
                return 100;
            }
    ...
    protected function _remove($file)
    {
        ...
        if (!@unlink($file)) {
            ...
            $this->_log("SpamTrawler_Cache_Backend_File::_remove() : we can't remove $file");
            return false;
        }
        return true;
    }
     ...
     * @return boolean True if no problem
     */
    protected function _clean($dir, $mode = SpamTrawler_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        if (!is_dir($dir)) {
            return false;
        }
            ...
            if (is_file($file)) {
                ...
                switch ($mode) {
                    ...
                    case 'matchingAny':
                        ...
                        break;
                    default:
                        SpamTrawler_Cache::throwException('Invalid mode for _get() method');
                        break;
                }
            }
            if ((is_dir($file)) and ($this->_options['hashed_directory_level']>0)) {
                ...
                if ($recursiveRs === false) {
                    $this->_log('SpamTrawler_Cache_Backend_File::_get() / recursive call : can\'t list entries of "'.$file.'"');
                } else {
    ...
    protected function _hash($data, $controlType)
    {
        ...
        default:
            SpamTrawler_Cache::throwException("Incorrect hash function : $controlType");
        }
    }
