...
class Firewall_ASNWhitelist_Model_Import extends Firewall_Abstract_Import_Import {
    protected $dbTableName = 'whitelist_asn';
    protected $sCheckValueDbField = 'asn';
    ...
    protected function validate($iLine){
        ...
        if ($this->sFilterMode == 'regex') {
            ...
            if(!preg_match('/^AS+[0-9]+$/', $this->sCheckValue)){
                $this->aErrors[] = '[Line: ' . $iLine . '] Invalid ASN';
                return false;
            }
        } else {
            if(!preg_match('/^AS+[0-9]+$/', $this->sCheckValue)){
