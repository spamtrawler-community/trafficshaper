 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Filter_Boolean implements SpamTrawler_Filter_Interface
{
    const BOOLEAN      = 1;
    const INTEGER      = 2;
    const FLOAT        = 4;
     ...
     * @return SpamTrawler_Filter_Boolean
     */
    public function setType($type = null)
    {
        ...
        if (!is_int($type) || ($type < 0) || ($type > self::ALL)) {
            ...
            throw new SpamTrawler_Filter_Exception('Unknown type');
        }
     ...
     *
     * @param  string|array|SpamTrawler_Locale $locale
     * @throws SpamTrawler_Filter_Exception
     * @return SpamTrawler_Filter_Boolean
     */
    public function setLocale($locale = null)
    {
        if (is_string($locale)) {
            $locale = array($locale);
        } elseif ($locale instanceof SpamTrawler_Locale) {
            $locale = array($locale->toString());
        } elseif (!is_array($locale)) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('Locale has to be string, array or an instance of SpamTrawler_Locale');
        }

        require_once 'SpamTrawler/Locale.php';
        foreach ($locale as $single) {
            if (!SpamTrawler_Locale::isLocale($single)) {
                require_once 'SpamTrawler/Filter/Exception.php';
                throw new SpamTrawler_Filter_Exception("Unknown locale '$single'");
            }
        }
    ...
    protected function _getLocalizedQuestion($value, $yes, $locale)
    {
        ...
        $str = SpamTrawler_Locale::getTranslation($question, 'question', $locale);
        $str = explode(':', $str);
        if (!empty($str)) {
            foreach($str as $no) {
