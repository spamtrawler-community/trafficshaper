...
/**
 * @see SpamTrawler_Filter_Encrypt_Interface
 */
require_once 'SpamTrawler/Filter/Encrypt/Interface.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Filter_Encrypt_Mcrypt implements SpamTrawler_Filter_Encrypt_Interface
{
    /**
     ...
     * Class constructor
     *
     * @param string|array|SpamTrawler_Config $options Cryption Options
     */
    public function __construct($options)
    {
        if (!extension_loaded('mcrypt')) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('This filter needs the mcrypt extension');
        }
        ...
        if ($options instanceof SpamTrawler_Config) {
            ...
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('Invalid options argument provided to filter');
        }
     ...
     * @return SpamTrawler_Filter_File_Encryption
     */
    public function setEncryption($options)
    {
        ...
        if (!in_array($options['algorithm'], $algorithms)) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception("The algorithm '{$options['algorithm']}' is not supported");
        }

        $modes = mcrypt_list_modes($options['mode_directory']);
        if (!in_array($options['mode'], $modes)) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception("The mode '{$options['mode']}' is not supported");
        }

        if (!mcrypt_module_self_test($options['algorithm'], $options['algorithm_directory'])) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('The given algorithm can not be used due an internal mcrypt problem');
        }
     ...
     * @return SpamTrawler_Filter_Encrypt_Mcrypt
     */
    public function setVector($vector = null)
    {
        ...
        if (empty($vector)) {
            ...
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('The given vector has a wrong size for the set algorithm');
        }
    ...
    public function encrypt($value)
    {
        ...
        if (!empty($this->_compression)) {
            require_once 'SpamTrawler/Filter/Compress.php';
            $compress = new SpamTrawler_Filter_Compress($this->_compression);
    ...
    public function decrypt($value)
    {
        ...
        if (!empty($this->_compression)) {
            require_once 'SpamTrawler/Filter/Decompress.php';
            $decompress = new SpamTrawler_Filter_Decompress($this->_compression);
     ...
     * Open a cipher
     *
     * @throws SpamTrawler_Filter_Exception When the cipher can not be opened
    ...
    protected function _openCipher()
    {
        ...
        if ($cipher === false) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('Mcrypt can not be opened with your settings');
        }
    ...
    protected function _initCipher($cipher)
    {
        ...
        if (empty($keysizes) || ($this->_encryption['salt'] == true)) {
            ...
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('The given key has a wrong size for the set algorithm');
        }

        $result = mcrypt_generic_init($cipher, $key, $this->_encryption['vector']);
        if ($result < 0) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('Mcrypt could not be initialize with the given setting');
        }
