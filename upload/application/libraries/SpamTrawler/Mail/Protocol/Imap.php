 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Mail_Protocol_Imap
{
    /**
    ...
    public function connect($host, $port = null, $ssl = false)
    {
        ...
        if (!$this->_socket) {
            ...
            throw new SpamTrawler_Mail_Protocol_Exception('cannot connect to host; error = ' . $errstr .
                                                   ' (errno = ' . $errno . ' )');
        }
        ...
        if (!$this->_assumedNextLine('* OK')) {
             ...
             */
            require_once 'SpamTrawler/Mail/Protocol/Exception.php';
            throw new SpamTrawler_Mail_Protocol_Exception('host doesn\'t allow connection');
        }
        ...
        if ($ssl === 'TLS') {
            ...
            if (!$result) {
                 ...
                 */
                require_once 'SpamTrawler/Mail/Protocol/Exception.php';
                throw new SpamTrawler_Mail_Protocol_Exception('cannot enable TLS');
            }
        }
    }
    ...
    protected function _nextLine()
    {
        ...
        if ($line === false) {
             ...
             */
            require_once 'SpamTrawler/Mail/Protocol/Exception.php';
            throw new SpamTrawler_Mail_Protocol_Exception('cannot read - connection closed?');
        }
        ...
        foreach ($tokens as $token) {
            if (is_array($token)) {
                if (@fputs($this->_socket, $line . ' ' . $token[0] . "\r\n") === false) {
                     ...
                     */
                    require_once 'SpamTrawler/Mail/Protocol/Exception.php';
                    throw new SpamTrawler_Mail_Protocol_Exception('cannot write - connection closed?');
                }
                if (!$this->_assumedNextLine('+ ')) {
                     ...
                     */
                    require_once 'SpamTrawler/Mail/Protocol/Exception.php';
                    throw new SpamTrawler_Mail_Protocol_Exception('cannot send literal string');
                }
                $line = $token[1];
            } else {
                $line .= ' ' . $token;
...
//              $result[] = $this->escapeString($v);
    ...
    public function fetch($items, $from, $to = null)
    {
        ...
        if ($to === null && !is_array($from)) {
             ...
             */
            require_once 'SpamTrawler/Mail/Protocol/Exception.php';
            throw new SpamTrawler_Mail_Protocol_Exception('the single id was not found in response');
        }
