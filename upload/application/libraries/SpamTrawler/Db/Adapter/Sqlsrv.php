...
/**
 * @see SpamTrawler_Db_Statement_Sqlsrv
 */
require_once 'SpamTrawler/Db/Statement/Sqlsrv.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Db_Adapter_Sqlsrv extends SpamTrawler_Db_Adapter_Abstract
{
    /**
     ...
     * SpamTrawler_Db::INT_TYPE, SpamTrawler_Db::BIGINT_TYPE, or SpamTrawler_Db::FLOAT_TYPE.
     *
     * Values are:
     * 0 = 32-bit integer
     * 1 = 64-bit integer
    ...
    protected $_numericDataTypes = array(
        SpamTrawler_Db::INT_TYPE    => SpamTrawler_Db::INT_TYPE,
        SpamTrawler_Db::BIGINT_TYPE => SpamTrawler_Db::BIGINT_TYPE,
        SpamTrawler_Db::FLOAT_TYPE  => SpamTrawler_Db::FLOAT_TYPE,
        'INT'                => SpamTrawler_Db::INT_TYPE,
        'SMALLINT'           => SpamTrawler_Db::INT_TYPE,
        'TINYINT'            => SpamTrawler_Db::INT_TYPE,
        'BIGINT'             => SpamTrawler_Db::BIGINT_TYPE,
        'DECIMAL'            => SpamTrawler_Db::FLOAT_TYPE,
        'FLOAT'              => SpamTrawler_Db::FLOAT_TYPE,
        'MONEY'              => SpamTrawler_Db::FLOAT_TYPE,
        'NUMERIC'            => SpamTrawler_Db::FLOAT_TYPE,
        'REAL'               => SpamTrawler_Db::FLOAT_TYPE,
        'SMALLMONEY'         => SpamTrawler_Db::FLOAT_TYPE,
    );
     ...
     * @var string
     */
    protected $_defaultStmtClass = 'SpamTrawler_Db_Statement_Sqlsrv';

    /**
     * Creates a connection resource.
     *
     * @return void
     * @throws SpamTrawler_Db_Adapter_Sqlsrv_Exception
     */
    protected function _connect()
    {
        ...
        if (!extension_loaded('sqlsrv')) {
            /**
             * @see SpamTrawler_Db_Adapter_Sqlsrv_Exception
             */
            require_once 'SpamTrawler/Db/Adapter/Sqlsrv/Exception.php';
            throw new SpamTrawler_Db_Adapter_Sqlsrv_Exception('The Sqlsrv extension is required for this adapter but the extension is not loaded');
        }
        ...
        if (!$this->_connection) {
             ...
             */
            require_once 'SpamTrawler/Db/Adapter/Sqlsrv/Exception.php';
            throw new SpamTrawler_Db_Adapter_Sqlsrv_Exception(sqlsrv_errors());
        }
    }
     ...
     *
     * @param array $config
     * @throws SpamTrawler_Db_Adapter_Exception
     */
    protected function _checkRequiredOptions(array $config)
    {
        // we need at least a dbname
        if (! array_key_exists('dbname', $config)) {
            /** @see SpamTrawler_Db_Adapter_Exception */
            require_once 'SpamTrawler/Db/Adapter/Exception.php';
            throw new SpamTrawler_Db_Adapter_Exception("Configuration array must have a key for 'dbname' that names the database instance");
        }
        ...
        if (! array_key_exists('password', $config) && array_key_exists('username', $config)) {
             ...
             */
            require_once 'SpamTrawler/Db/Adapter/Exception.php';
            throw new SpamTrawler_Db_Adapter_Exception("Configuration array must have a key for 'password' for login credentials.
        ...
        if (array_key_exists('password', $config) && !array_key_exists('username', $config)) {
             ...
             */
            require_once 'SpamTrawler/Db/Adapter/Exception.php';
            throw new SpamTrawler_Db_Adapter_Exception("Configuration array must have a key for 'username' for login credentials.
                                                If Windows Authentication is desired, both keys 'username' and 'password' should be ommited from config.");
        }
    }
    ...
    public function setTransactionIsolationLevel($level = null)
    {
        ...
        switch ($level) {
            ...
            default:
                require_once 'SpamTrawler/Db/Adapter/Sqlsrv/Exception.php';
                throw new SpamTrawler_Db_Adapter_Sqlsrv_Exception("Invalid transaction isolation level mode '$level' specified");
        }

        if (!sqlsrv_query($this->_connection, "SET TRANSACTION ISOLATION LEVEL $sql;")) {
            require_once 'SpamTrawler/Db/Adapter/Sqlsrv/Exception.php';
            throw new SpamTrawler_Db_Adapter_Sqlsrv_Exception("Transaction cannot be changed to '$level'");
        }
     ...
     * @return SpamTrawler_Db_Statement_Sqlsrv
     */
    public function prepare($sql)
    {
        ...
        if (!class_exists($stmtClass)) {
            ...
            SpamTrawler_Loader::loadClass($stmtClass);
        }
    ...
    public function describeTable($tableName, $schemaName = null)
    {
        ...
        $stmt   = $this->query($sql);
        $result = $stmt->fetchAll(SpamTrawler_Db::FETCH_NUM);
        ...
        $primaryKeysResult = $stmt->fetchAll(SpamTrawler_Db::FETCH_NUM);
    ...
    {
        switch ($mode) {
            case SpamTrawler_Db::FETCH_NUM:   // seq array
            case SpamTrawler_Db::FETCH_ASSOC: // assoc array
            case SpamTrawler_Db::FETCH_BOTH:  // seq+assoc array
            case SpamTrawler_Db::FETCH_OBJ:   // object
                $this->_fetchMode = $mode;
                break;
            case SpamTrawler_Db::FETCH_BOUND: // bound to PHP variable
                require_once 'SpamTrawler/Db/Adapter/Sqlsrv/Exception.php';
                throw new SpamTrawler_Db_Adapter_Sqlsrv_Exception('FETCH_BOUND is not supported yet');
                break;
            default:
                require_once 'SpamTrawler/Db/Adapter/Sqlsrv/Exception.php';
                throw new SpamTrawler_Db_Adapter_Sqlsrv_Exception("Invalid fetch mode '$mode' specified");
                break;
        }
    }
     ...
     */
     public function limit($sql, $count, $offset = 0)
     {
        ...
        if ($count <= 0) {
            require_once 'SpamTrawler/Db/Adapter/Exception.php';
            throw new SpamTrawler_Db_Adapter_Exception("LIMIT argument count=$count is not valid");
        }
        ...
        if ($offset < 0) {
            /** @see SpamTrawler_Db_Adapter_Exception */
            require_once 'SpamTrawler/Db/Adapter/Exception.php';
            throw new SpamTrawler_Db_Adapter_Exception("LIMIT argument offset=$offset is not valid");
        }
