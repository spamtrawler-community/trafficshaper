 ...
 * @category   SpamTrawler
 * @package    SpamTrawler_Date
 * @subpackage SpamTrawler_Date_DateObject
 * @copyright  Copyright (c) 2005-2014 SpamTrawler Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class SpamTrawler_Date_DateObject {
    ...
    protected function setUnixTimestamp($timestamp = null)
    {
        ...
        if (is_numeric($timestamp)) {
            ...
            throw new SpamTrawler_Date_Exception('\'' . $timestamp . '\' is not a valid UNIX timestamp', 0, null, $timestamp);
        }
    ...
    /**
     * Internal mktime function used by SpamTrawler_Date.
    ...
    protected function mktime($hour, $minute, $second, $month, $day, $year, $gmt = false)
    {
        ...
        if (isset(self::$_cache)) {
            $id = strtr('SpamTrawler_DateObject_mkTime_' . $this->_offset . '_' . $year.$month.$day.'_'.$hour.$minute.$second . '_'.(int)$gmt, '-','_');
        ...
        if (isset(self::$_cache)) {
            if (self::$_cacheTags) {
                self::$_cache->save( serialize($date), $id, array('SpamTrawler_Date'));
            } else {
    ...
    /**
     * Internal mktime function used by SpamTrawler_Date for handling 64bit timestamps.
     *
    ...
    protected function date($format, $timestamp = null, $gmt = false)
    {
        ...
        $origstamp = $timestamp;
        if (isset(self::$_cache)) {
            $idstamp = strtr('SpamTrawler_DateObject_date_' . $this->_offset . '_'. $timestamp . '_'.(int)$gmt, '-','_');
        ...
        if (empty($gmt) and empty($jump)) {
            ...
            if (isset(self::$_cache)) {
                if (self::$_cacheTags) {
                    self::$_cache->save( serialize($timestamp), $idstamp, array('SpamTrawler_Date'));
                } else {
    ...
    protected function getDateParts($timestamp = null, $fast = null)
    {
        ...
        if (isset(self::$_cache)) {
            $id = strtr('SpamTrawler_DateObject_getDateParts_' . $timestamp.'_'.(int)$fast, '-','_');
            if ($result = self::$_cache->load($id)) {
                return unserialize($result);
            }
        }
        ...
        if (isset(self::$_cache)) {
            if (self::$_cacheTags) {
                self::$_cache->save( serialize($array), $id, array('SpamTrawler_Date'));
            } else {
     ...
     * @return SpamTrawler_Date_DateObject Provides fluent interface
     * @throws SpamTrawler_Date_Exception
     */
    public function setTimezone($zone = null)
    {
        ...
        if (function_exists('timezone_open')) {
            if (!@timezone_open($zone)) {
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("timezone ($zone) is not a known timezone", 0, null, $zone);
            }
        }
