...
/**
 * @see SpamTrawler_Config_Xml
 */
require_once 'SpamTrawler/Config/Xml.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Config_Writer_Xml extends SpamTrawler_Config_Writer_FileAbstract
{
    /**
     * Render a SpamTrawler_Config into a XML config string.
     *
     * @since 1.10
     * @return string
     */
    ...
    {
        $xml         = new SimpleXMLElement('<zend-config xmlns:zf="' . SpamTrawler_Config_Xml::XML_NAMESPACE . '"/>');
        ...
        if (is_string($sectionName)) {
            ...
            foreach ($this->_config as $sectionName => $data) {
                if (!($data instanceof SpamTrawler_Config)) {
                    $xml->addChild($sectionName, (string) $data);
                } else {
                    $child = $xml->addChild($sectionName);

                    if (isset($extends[$sectionName])) {
                        $child->addAttribute('zf:extends', $extends[$sectionName], SpamTrawler_Config_Xml::XML_NAMESPACE);
                    }
     ...
     * @return void
     */
    protected function _addBranch(SpamTrawler_Config $config, SimpleXMLElement $xml, SimpleXMLElement $parent)
    {
        $branchType = null;
        ...
        foreach ($config as $key => $value) {
            if ($branchType === null) {
                ...
                throw new SpamTrawler_Config_Exception('Mixing of string and numeric keys is not allowed');
            }
