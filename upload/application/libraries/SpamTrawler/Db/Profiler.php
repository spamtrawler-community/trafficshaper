...
class SpamTrawler_Db_Profiler
{
    ...
    /**
     * Array of SpamTrawler_Db_Profiler_Query objects.
     *
     * @var array
     */
     ...
     * @return SpamTrawler_Db_Profiler Provides a fluent interface
     */
     ...
     * the SpamTrawler_Db_Profiler::* constants to only save profiles for that type of
     ...
     *
     * @return integer|null
     * @see    SpamTrawler_Db_Profiler::setFilterQueryType()
     */
     ...
     *
     * @param  SpamTrawler_Db_Profiler_Query $query
     * @return integer or null
     */
    public function queryClone(SpamTrawler_Db_Profiler_Query $query)
    {
    ...
    /**
     * Starts a query.  Creates a new query profile object (SpamTrawler_Db_Profiler_Query)
     ...
     * @param  integer $queryType   OPTIONAL Type of query, one of the SpamTrawler_Db_Profiler::* constants
     * @return integer|null
     */
    public function queryStart($queryText, $queryType = null)
    {
        ...
        /**
         * @see SpamTrawler_Db_Profiler_Query
         */
        require_once 'SpamTrawler/Db/Profiler/Query.php';
        $this->_queryProfiles[] = new SpamTrawler_Db_Profiler_Query($queryText, $queryType);
     ...
     * @throws SpamTrawler_Db_Profiler_Exception
     * @return string   Inform that a query is stored or ignored.
     */
    public function queryEnd($queryId)
    {
        // Don't do anything if the SpamTrawler_Db_Profiler is not enabled.
        if (!$this->_enabled) {
            return self::IGNORED;
        }
        ...
        if (!isset($this->_queryProfiles[$queryId])) {
            /**
             * @see SpamTrawler_Db_Profiler_Exception
             */
            require_once 'SpamTrawler/Db/Profiler/Exception.php';
            throw new SpamTrawler_Db_Profiler_Exception("Profiler has no query with handle '$queryId'.");
        }
        ...
        if ($qp->hasEnded()) {
             ...
             */
            require_once 'SpamTrawler/Db/Profiler/Exception.php';
            throw new SpamTrawler_Db_Profiler_Exception("Query with profiler handle '$queryId' has already ended.");
        }
     ...
     * by queryStart() and it will return a SpamTrawler_Db_Profiler_Query object.
     *
     * @param  integer $queryId
     * @throws SpamTrawler_Db_Profiler_Exception
     * @return SpamTrawler_Db_Profiler_Query
     */
    public function getQueryProfile($queryId)
    {
        if (!array_key_exists($queryId, $this->_queryProfiles)) {
             ...
             */
            require_once 'SpamTrawler/Db/Profiler/Exception.php';
            throw new SpamTrawler_Db_Profiler_Exception("Query handle '$queryId' not found in profiler log.");
        }
    ...
    /**
     * Get an array of query profiles (SpamTrawler_Db_Profiler_Query objects).  If $queryType
     * is set to one of the SpamTrawler_Db_Profiler::* constants then only queries of that
     ...
     * one or more of the SpamTrawler_Db_Profiler::* constants, the elapsed time will be calculated
     ...
     * be counted.  If $queryType is set to one of the SpamTrawler_Db_Profiler::* constants, only queries of
    ...
    /**
     * Get the SpamTrawler_Db_Profiler_Query object for the last query that was run, regardless if it has
     * ended or not.  If the query has not ended, its end time will be null.  If no queries have
     * been profiled, false is returned.
     *
     * @return SpamTrawler_Db_Profiler_Query|false
     */
