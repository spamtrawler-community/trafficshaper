...
/** SpamTrawler_Loader_Autoloader_Interface */
require_once 'SpamTrawler/Loader/Autoloader/Interface.php';

/**
 * Resource loader
 *
 * @uses       SpamTrawler_Loader_Autoloader_Interface
 * @package    SpamTrawler_Loader
 * @subpackage Autoloader
 * @copyright  Copyright (c) 2005-2014 SpamTrawler Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Loader_Autoloader_Resource implements SpamTrawler_Loader_Autoloader_Interface
{
    /**
     ...
     * Constructor
     *
     * @param  array|SpamTrawler_Config $options Configuration options for resource autoloader
     * @return void
     */
    public function __construct($options)
    {
        ...
        if (!is_array($options)) {
            require_once 'SpamTrawler/Loader/Exception.php';
            throw new SpamTrawler_Loader_Exception('Options must be passed to resource loader constructor');
        }
        ...
        if ((null === $namespace)
            ...
            require_once 'SpamTrawler/Loader/Exception.php';
            throw new SpamTrawler_Loader_Exception('Resource loader requires both a namespace and a base path for initialization');
        }

        if (!empty($namespace)) {
            $namespace .= '_';
        }
        require_once 'SpamTrawler/Loader/Autoloader.php';
        SpamTrawler_Loader_Autoloader::getInstance()->unshiftAutoloader($this, $namespace);
    }
     ...
     * syntax. Example:
     * <code>
     * $loader = new SpamTrawler_Loader_Autoloader_Resource(array(
     *     'namespace' => 'Stuff_',
     ...
     * @param  array $args
     * @return mixed
     * @throws SpamTrawler_Loader_Exception if method not beginning with 'get' or not matching a valid resource type is called
     */
    public function __call($method, $args)
    {
        if ('get' == substr($method, 0, 3)) {
            ...
            if (!$this->hasResourceType($type)) {
                require_once 'SpamTrawler/Loader/Exception.php';
                throw new SpamTrawler_Loader_Exception("Invalid resource type $type; cannot load resource");
            }
            if (empty($args)) {
                require_once 'SpamTrawler/Loader/Exception.php';
                throw new SpamTrawler_Loader_Exception("Cannot load resources; no resource specified");
            }
        ...
        require_once 'SpamTrawler/Loader/Exception.php';
        throw new SpamTrawler_Loader_Exception("Method '$method' is not supported");
    }
    ...
    public function getClassPath($class)
    {
        ...
        if (SpamTrawler_Loader::isReadable($classPath)) {
            return $classPath;
        }
     ...
     *
     * @param  array $options
     * @return SpamTrawler_Loader_Autoloader_Resource
     */
    ...
    public function addResourceType($type, $path, $namespace = null)
    {
        ...
        if (!isset($this->_resourceTypes[$type])) {
            if (null === $namespace) {
                require_once 'SpamTrawler/Loader/Exception.php';
                throw new SpamTrawler_Loader_Exception('Initial definition of a resource type must include a namespace');
            }
            $namespaceTopLevel = $this->getNamespace();
        ...
        if (!is_string($path)) {
            require_once 'SpamTrawler/Loader/Exception.php';
            throw new SpamTrawler_Loader_Exception('Invalid path specification provided; must be string');
        }
    ...
    public function addResourceTypes(array $types)
    {
        foreach ($types as $type => $spec) {
            if (!is_array($spec)) {
                require_once 'SpamTrawler/Loader/Exception.php';
                throw new SpamTrawler_Loader_Exception('addResourceTypes() expects an array of arrays');
            }
            if (!isset($spec['path'])) {
                require_once 'SpamTrawler/Loader/Exception.php';
                throw new SpamTrawler_Loader_Exception('addResourceTypes() expects each array to include a paths element');
            }
            $paths  = $spec['path'];
            $namespace = null;
     ...
     *
     * @see    SpamTrawler_Loader_Autoloader_Resource::addResourceTypes()
     * @param  array $types
     * @return SpamTrawler_Loader_Autoloader_Resource
     */
     ...
     * @param  string $type
     * @return object
     * @throws SpamTrawler_Loader_Exception if resource type not specified or invalid
     */
    public function load($resource, $type = null)
    {
        if (null === $type) {
            ...
            if (empty($type)) {
                require_once 'SpamTrawler/Loader/Exception.php';
                throw new SpamTrawler_Loader_Exception('No resource type specified');
            }
        }
        if (!$this->hasResourceType($type)) {
            require_once 'SpamTrawler/Loader/Exception.php';
            throw new SpamTrawler_Loader_Exception('Invalid resource type specified');
        }
        $namespace = $this->_resourceTypes[$type]['namespace'];
