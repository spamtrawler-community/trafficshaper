...
class Firewall_AntiWebSpam_Model_Manage
{
    private $dbTableName = 'antiwebspam_urls';
    public $oInput = NULL;
    ...
    public static function validateSettings($aSettings){
        ...
        if(!ctype_digit($aSettings['conf_status'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'AntiWebSpam Status Invalid!';
        }

        if(!ctype_digit($aSettings['conf_params']['allowcaptcha'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'AntiWebSpam Allow Captcha Invalid!';
        }

        /*
        if(!ctype_digit($aSettings['conf_order'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'AntiWebSpam Order Invalid!';
        }
        */
        ...
        if(!ctype_alnum($sBlockReason)){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'AntiWebSpam Block Reason Invalid!';
        }

        $aAllowedFilterTypes = array('hostnames','ips','emails');
        if(isset($aSettings['conf_params']['filter_types']) && array_diff($aSettings['conf_params']['filter_types'], $aAllowedFilterTypes)) {
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'AntiWebSpam Filter Types Invalid!';
        }
