...
/**
 * @see SpamTrawler_Db_Statement_Interface
 */
require_once 'SpamTrawler/Db/Statement/Interface.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class SpamTrawler_Db_Statement implements SpamTrawler_Db_Statement_Interface
{
     ...
     * @var integer
     */
    protected $_fetchMode = SpamTrawler_Db::FETCH_ASSOC;
    ...
    /**
     * @var SpamTrawler_Db_Profiler_Query
     */
     ...
     * @param mixed $sql Either a string or SpamTrawler_Db_Select.
     */
    public function __construct($adapter, $sql)
    {
        $this->_adapter = $adapter;
        if ($sql instanceof SpamTrawler_Db_Select) {
            $sql = $sql->assemble();
        }
        ...
        foreach ($this->_sqlSplit as $key => $val) {
            if ($val == '?') {
                if ($this->_adapter->supportsParameters('positional') === false) {
                    ...
                    throw new SpamTrawler_Db_Statement_Exception("Invalid bind-variable position '$val'");
                }
            } else if ($val[0] == ':') {
                if ($this->_adapter->supportsParameters('named') === false) {
                     ...
                     */
                    require_once 'SpamTrawler/Db/Statement/Exception.php';
                    throw new SpamTrawler_Db_Statement_Exception("Invalid bind-variable name '$val'");
                }
            }
            $this->_sqlParam[] = $val;
        }
    ...
    public function bindParam($parameter, &$variable, $type = null, $length = null, $options = null)
    {
        if (!is_int($parameter) && !is_string($parameter)) {
             ...
             */
            require_once 'SpamTrawler/Db/Statement/Exception.php';
            throw new SpamTrawler_Db_Statement_Exception('Invalid bind-variable position');
        }
        ...
        if ($position === null) {
             ...
             */
            require_once 'SpamTrawler/Db/Statement/Exception.php';
            throw new SpamTrawler_Db_Statement_Exception("Invalid bind-variable position '$parameter'");
        }
    ...
    {
        $data = array();
        if ($style === SpamTrawler_Db::FETCH_COLUMN && $col === null) {
            $col = 0;
        }
        if ($col === null) {
    ...
    public function fetchColumn($col = 0)
    {
        $data = array();
        $col = (int) $col;
        $row = $this->fetch(SpamTrawler_Db::FETCH_NUM);
        if (!is_array($row)) {
            return false;
        }
        return $row[$col];
    ...
    public function fetchObject($class = 'stdClass', array $config = array())
    {
        ...
        $row = $this->fetch(SpamTrawler_Db::FETCH_ASSOC);
        if (!is_array($row)) {
            return false;
        }
     ...
     * @return bool
     * @throws SpamTrawler_Db_Statement_Exception
     */
    public function setFetchMode($mode)
    {
        switch ($mode) {
            ...
            case SpamTrawler_Db::FETCH_OBJ:
                ...
                break;
            case SpamTrawler_Db::FETCH_BOUND:
            default:
                $this->closeCursor();
                /**
                 * @see SpamTrawler_Db_Statement_Exception
                 */
                require_once 'SpamTrawler/Db/Statement/Exception.php';
                throw new SpamTrawler_Db_Statement_Exception('invalid fetch mode');
                break;
        }
    }
     ...
     * particular SpamTrawler_Db_Statement object.
     *
