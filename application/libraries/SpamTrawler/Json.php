...
/**
 * SpamTrawler_Json_Expr.
 *
 * @see SpamTrawler_Json_Expr
 */
require_once 'SpamTrawler/Json/Expr.php';

/** @see SpamTrawler_Xml_Security */
require_once 'SpamTrawler/Xml/Security.php';
 ...
 * @category   SpamTrawler
 * @package    SpamTrawler_Json
 * @uses       SpamTrawler_Json_Expr
 * @copyright  Copyright (c) 2005-2014 SpamTrawler Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Json
{
    /**
     ...
     * objects. See {@link SpamTrawler_Json_Decoder::decode()} for details.
     * @return mixed
     */
    public static function decode($encodedValue, $objectDecodeType = SpamTrawler_Json::TYPE_ARRAY)
    {
        ...
        if (function_exists('json_decode') && self::$useBuiltinEncoderDecoder !== true) {
            ...
            if (!function_exists('json_last_error')) {
                if (strtolower($encodedValue) === 'null') {
                    ...
                    require_once 'SpamTrawler/Json/Exception.php';
                    throw new SpamTrawler_Json_Exception('Decoding failed');
                }
            // php >= 5.3
                ...
                switch ($jsonLastErr) {
                    case JSON_ERROR_DEPTH:
                        throw new SpamTrawler_Json_Exception('Decoding failed: Maximum stack depth exceeded');
                    case JSON_ERROR_CTRL_CHAR:
                        throw new SpamTrawler_Json_Exception('Decoding failed: Unexpected control character found');
                    case JSON_ERROR_SYNTAX:
                        throw new SpamTrawler_Json_Exception('Decoding failed: Syntax error');
                    default:
                        throw new SpamTrawler_Json_Exception('Decoding failed');
                }
            }
        ...
        require_once 'SpamTrawler/Json/Decoder.php';
        return SpamTrawler_Json_Decoder::decode($encodedValue, $objectDecodeType);
    }
     ...
     *
     * NOTE: Encoding native javascript expressions are possible using SpamTrawler_Json_Expr.
    ...
    public static function encode($valueToEncode, $cycleCheck = false, $options = array())
    {
        ...
        // Pre-encoding look for SpamTrawler_Json_Expr objects and replacing by tmp ids
        $javascriptExpressions = array();
        if(isset($options['enableJsonExprFinder'])
           && ($options['enableJsonExprFinder'] == true)
        ) {
            /**
             * @see SpamTrawler_Json_Encoder
             */
            require_once "SpamTrawler/Json/Encoder.php";
        ...
        if (function_exists('json_encode') && self::$useBuiltinEncoderDecoder !== true) {
            ...
            require_once 'SpamTrawler/Json/Encoder.php';
            $encodedResult = SpamTrawler_Json_Encoder::encode($valueToEncode, $cycleCheck, $options);
        }

        //only do post-proccessing to revert back the SpamTrawler_Json_Expr if any.
    ...
    /**
     * Check & Replace SpamTrawler_Json_Expr for tmp ids in the valueToEncode
     *
     * Check if the value is a SpamTrawler_Json_Expr, and if replace its value
     ...
     *
     * @see encode
     * @param array|object|SpamTrawler_Json_Expr $value a string - object property to be encoded
    ...
    {
         if ($value instanceof SpamTrawler_Json_Expr) {
            // TODO: Optimize with ascii keys, if performance is bad
            $magicKey = "____" . $currentKey . "_" . (count($javascriptExpressions));
            $javascriptExpressions[] = array(

                //if currentKey is integer, encodeUnicodeString call is not required.
                "magicKey" => (is_int($currentKey)) ? $magicKey : SpamTrawler_Json_Encoder::encodeUnicodeString($magicKey),
     ...
     * the XML tags
     *
     * In order to allow SpamTrawler_Json_Expr from xml, we check if the node
     * matchs the pattern that try to detect if it is a new SpamTrawler_Json_Expr
     * if it matches, we return a new SpamTrawler_Json_Expr instead of a text node
     *
     * @param SimpleXMLElement $simpleXmlElementObject
     * @return SpamTrawler_Json_Expr|string
     */
    protected static function _getXmlValue($simpleXmlElementObject) {
        $pattern = '/^[\s]*new SpamTrawler_Json_Expr[\s]*\([\s]*[\"\']{1}(.*)[\"\']{1}[\s]*\)[\s]*$/';
        $matchings = array();
        $match = preg_match ($pattern, $simpleXmlElementObject, $matchings);
        if ($match) {
            return new SpamTrawler_Json_Expr($matchings[1]);
        } else {
     ...
     *
     * Throws a SpamTrawler_Json_Exception if the XML tree is deeper than the allowed limit.
     *
     * @param SimpleXMLElement $simpleXmlElementObject
    ...
    protected static function _processXml($simpleXmlElementObject, $ignoreXmlAttributes, $recursionDepth=0)
    {
        ...
        if ($recursionDepth > self::$maxRecursionDepthAllowed) {
            ...
            require_once 'SpamTrawler/Json/Exception.php';
            throw new SpamTrawler_Json_Exception(
     ...
     *
     * Throws a SpamTrawler_Json_Exception if the input not a XML formatted string.
     * NOTE: Encoding native javascript expressions via SpamTrawler_Json_Expr is not possible.
     *
     * @static
     * @access public
     ...
     * @throws SpamTrawler_Json_Exception
     */
    public static function fromXml($xmlStringContents, $ignoreXmlAttributes=true)
    {
        // Load the XML formatted string into a Simple XML Element object.
        $simpleXmlElementObject = SpamTrawler_Xml_Security::scan($xmlStringContents);

        // If it is not a valid XML content, throw an exception.
        if ($simpleXmlElementObject == null) {
            require_once 'SpamTrawler/Json/Exception.php';
            throw new SpamTrawler_Json_Exception('Function fromXml was called with an invalid XML formatted string.');
        ...
        // Convert the PHP array to JSON using SpamTrawler_Json encode method.
