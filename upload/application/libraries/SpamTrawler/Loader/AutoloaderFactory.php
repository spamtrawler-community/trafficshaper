...
if (class_exists('SpamTrawler_Loader_AutoloaderFactory')) return;
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class SpamTrawler_Loader_AutoloaderFactory
{
    const STANDARD_AUTOLOADER = 'SpamTrawler_Loader_StandardAutoloader';
    ...
    /**
     * @var SpamTrawler_Loader_StandardAutoloader StandardAutoloader instance for resolving
     ...
     * the SpamTrawler library, using PSR-0 rules (unless the class has already been
     * loaded).
     *
     * @param  array|Traversable $options (optional) options to use. Defaults to SpamTrawler_Loader_StandardAutoloader
     * @return void
     * @throws SpamTrawler_Loader_Exception_InvalidArgumentException for invalid options
     * @throws SpamTrawler_Loader_Exception_InvalidArgumentException for unloadable autoloader classes
     */
    public static function factory($options = null)
    {
        ...
        if (!is_array($options) && !($options instanceof Traversable)) {
            ...
            throw new SpamTrawler_Loader_Exception_InvalidArgumentException(
        ...
        foreach ($options as $class => $options) {
            if (!isset(self::$loaders[$class])) {
                ...
                if (!class_exists($class) && !$autoloader->autoload($class)) {
                    require_once 'Exception/InvalidArgumentException.php';
                    throw new SpamTrawler_Loader_Exception_InvalidArgumentException(sprintf(
                ...
                if (version_compare(PHP_VERSION, '5.3.7', '>=')) {
                        if (!is_subclass_of($class, 'SpamTrawler_Loader_SplAutoloader')) {
                        require_once 'Exception/InvalidArgumentException.php';
                        throw new SpamTrawler_Loader_Exception_InvalidArgumentException(sprintf(
                            'Autoloader class %s must implement SpamTrawler\\Loader\\SplAutoloader',
                            $class
                        ));
                    }
                }
     ...
     *
     * @param string $class
     * @return SpamTrawler_Loader_SplAutoloader
     * @throws SpamTrawler_Loader_Exception_InvalidArgumentException for non-registered class
     */
    public static function getRegisteredAutoloader($class)
    {
        if (!isset(self::$loaders[$class])) {
            require_once 'Exception/InvalidArgumentException.php';
            throw new SpamTrawler_Loader_Exception_InvalidArgumentException(sprintf('Autoloader class "%s" not loaded', $class));
        }
     ...
     * allow resolving autoloaders not under the "SpamTrawler" or "SpamTrawler" namespaces.
     * 
     * @return SpamTrawler_Loader_SplAutoloader
     */
    protected static function getStandardAutoloader()
    {
        ...
        $loader = new SpamTrawler_Loader_StandardAutoloader();
