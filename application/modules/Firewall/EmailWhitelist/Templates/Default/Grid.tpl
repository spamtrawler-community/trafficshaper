...
<div class="row">
    <div class="small-4 columns">
        <input type="text" id="query_emailwhitelist" class="radius" value="" placeholder="Search">
    </div>
    <div class="small-2 columns">
        <select id="column_emailwhitelist" class="radius">
            <option value="email">Email</option>
            <option value="comment">Comment</option>
        </select>
    </div>
    <div class="small-2 columns">
        <select id="operator_emailwhitelist" class="radius">
    ...
    <div class="small-2 columns">
        <input type="button" id="btnFilter_emailwhitelist" class="k-button" value="Filter">
        <input type="button" id="btnReset_emailwhitelist" class="k-button" value="Reset">
    </div>
</div>

<div id="gridEmailWhitelist"></div>

        {literal}
        <script>
            $(document).ready(function () {
                var crudServiceBaseUrl = "{/literal}{$requestprotocol}://{$linkurl}Firewall/EmailWhitelist/Manage{literal}",
                ...
                $("#gridEmailWhitelist").kendoGrid({
                    ...
                    detailTemplate: kendo.template($("#commentTemplateEmailWhitelist").html()),
                    toolbar: ["create"],
                    columns: [
                        {
                            field: "email",
                ...
                $("#btnFilter_emailwhitelist").on("click", function(e) {
                    var ds = $("#gridEmailWhitelist").data("kendoGrid").dataSource;
                    ds.filter([{
                        "field": $("#column_emailwhitelist").val(),
                        "operator": $("#operator_emailwhitelist").val(),
                        "value": $("#query_emailwhitelist").val()}
                    ]);
                });

                $("#btnReset_emailwhitelist").on("click", function(e) {
                    $("#query_emailwhitelist").val('');
                    var ds = $("#gridEmailWhitelist").data("kendoGrid").dataSource;
                    ds.filter([]);
                });
            });
...
<!-- popup editor template -->
    <script id="popup_editorEmailWhitelist" type="text/x-kendo-template">
       <div id="errorsEmailWhitelist">Shows Errors here!</div>
    </script>

        <script type="text/x-kendo-template" id="commentTemplateEmailWhitelist">
            <div class="row">
                <div>Comment:</div>
