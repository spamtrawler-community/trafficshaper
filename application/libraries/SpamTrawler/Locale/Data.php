...
class SpamTrawler_Locale_Data
{
    ...
    private static function _findRoute($locale, $path, $attribute, $value, &$temp)
    {
        ...
        if (empty(self::$_ldml[(string) $locale])) {
            ...
            if (!file_exists($filename)) {
                ...
                throw new SpamTrawler_Locale_Exception("Missing locale file '$filename' for '$locale' locale.");
            }

            self::$_ldml[(string) $locale] = SpamTrawler_Xml_Security::scanFile($filename);
        }
     ...
     *
     * @param string|SpamTrawler_Locale $locale Locale to check
     * @return string
     */
    private static function _checkLocale($locale)
    {
        ...
        if (!(SpamTrawler_Locale::isLocale((string) $locale, null, false))) {
            require_once 'SpamTrawler/Locale/Exception.php';
            throw new SpamTrawler_Locale_Exception("Locale (" . (string) $locale . ") is a unknown locale");
        }

        if (SpamTrawler_Locale::isAlias($locale)) {
            // Return a valid CLDR locale so that the XML file can be loaded.
            return SpamTrawler_Locale::getAlias($locale);
        }
    ...
    public static function getList($locale, $path, $value = false)
    {
        ...
        if (!isset(self::$_cache) && !self::$_cacheDisabled) {
            require_once 'SpamTrawler/Cache.php';
            self::$_cache = SpamTrawler_Cache::factory(
                'Core',
                'File',
        ...
        $val = urlencode($val);
        $id = strtr('SpamTrawler_LocaleL_' . $locale . '_' . $path . '_' . $val, array('-' => '_', '%' => '_', '+' => '_'));
        ...
        switch(strtolower($path)) {
            ...
            default :
                require_once 'SpamTrawler/Locale/Exception.php';
                throw new SpamTrawler_Locale_Exception("Unknown list ($path) for parsing locale data.");
                break;
        }

        if (isset(self::$_cache)) {
            if (self::$_cacheTags) {
                self::$_cache->save( serialize($temp), $id, array('SpamTrawler_Locale'));
            } else {
    ...
    public static function getContent($locale, $path, $value = false)
    {
        ...
        $val = urlencode($val);
        $id = strtr('SpamTrawler_LocaleC_' . $locale . '_' . $path . '_' . $val, array('-' => '_', '%' => '_', '+' => '_'));
        if (!self::$_cacheDisabled && ($result = self::$_cache->load($id))) {
            return unserialize($result);
        }
        ...
        switch(strtolower($path)) {
            ...
            case 'defaultcalendar':
                $givenLocale = new SpamTrawler_Locale($locale);
            ...
            default :
                require_once 'SpamTrawler/Locale/Exception.php';
                throw new SpamTrawler_Locale_Exception("Unknown detail ($path) for parsing locale data.");
                break;
        }
    ...
    /**
     * Set a cache for SpamTrawler_Locale_Data
     *
     * @param SpamTrawler_Cache_Core $cache A cache frontend
     */
    ...
    {
        if (self::$_cacheTags) {
            self::$_cache->clean(SpamTrawler_Cache::CLEANING_MODE_MATCHING_TAG, array('SpamTrawler_Locale'));
        } else {
            self::$_cache->clean(SpamTrawler_Cache::CLEANING_MODE_ALL);
        }
    }
     ...
     *
     * @param SpamTrawler_Cache $cache
     */
    private static function _getTagSupportForCache()
    {
        $backend = self::$_cache->getBackend();
        if ($backend instanceof SpamTrawler_Cache_Backend_ExtendedInterface) {
