 ...
 *
 * @uses       SpamTrawler_Loader_Autoloader
 * @package    SpamTrawler_Loader
 * @subpackage Autoloader
 * @copyright  Copyright (c) 2005-2014 SpamTrawler Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Loader_Autoloader
{
    /**
     * @var SpamTrawler_Loader_Autoloader Singleton instance
     */
     ...
     */
    protected $_defaultAutoloader = array('SpamTrawler_Loader', 'loadClass');
    ...
    /**
     * @var array Supported namespaces 'SpamTrawler' and 'SpamTrawlerX' by default.
     */
     ...
     *
     * @return SpamTrawler_Loader_Autoloader
     */
    ...
    public static function autoload($class)
    {
        ...
        foreach ($self->getClassAutoloaders($class) as $autoloader) {
            if ($autoloader instanceof SpamTrawler_Loader_Autoloader_Interface) {
    ...
    public function setDefaultAutoloader($callback)
    {
        if (!is_callable($callback)) {
            throw new SpamTrawler_Loader_Exception('Invalid callback specified for default autoloader');
        }
     ...
     *
     * @param  array $autoloaders Array of PHP callbacks (or SpamTrawler_Loader_Autoloader_Interface implementations) to act as autoloaders
     * @return SpamTrawler_Loader_Autoloader
     */
    ...
    public function registerNamespace($namespace)
    {
        if (is_string($namespace)) {
            ...
            throw new SpamTrawler_Loader_Exception('Invalid namespace provided');
        }
    ...
    public function setZfPath($spec, $version = 'latest')
    {
        ...
        if (is_array($spec)) {
            if (!isset($spec['path'])) {
                throw new SpamTrawler_Loader_Exception('No path specified for ZF');
            }
            $path = $spec['path'];
     ...
     * @return bool|SpamTrawler_Loader_Autoloader Returns boolean if no argument is passed, object instance otherwise
     */
     ...
     *
     * @param  object|array|string $callback PHP callback or SpamTrawler_Loader_Autoloader_Interface implementation
    ...
    protected function _getVersionPath($path, $version)
    {
        ...
        if (empty($availableVersions)) {
            throw new SpamTrawler_Loader_Exception('No valid ZF installations discovered');
        }
     ...
     * @throws SpamTrawler_Loader_Exception if version string contains too many dots
     */
    protected function _getVersionType($version)
    {
        ...
        if (3 < $count) {
            throw new SpamTrawler_Loader_Exception('Invalid version string provided');
        }
        return 'specific';
    }
    ...
    {
        if (!is_dir($path)) {
            throw new SpamTrawler_Loader_Exception('Invalid ZF path provided');
        }
        ...
        foreach ((array) $dirs as $dir) {
            ...
            if (!preg_match('/^(?:SpamTrawlerFramework-)?(\d+\.\d+\.\d+((a|b|pl|pr|p|rc)\d+)?)(?:-minimal)?$/i', $dirName, $matches)) {
                continue;
            }
