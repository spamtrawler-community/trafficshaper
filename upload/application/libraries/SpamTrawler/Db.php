 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Db
{

    /**
     * Use the PROFILER constant in the config of a SpamTrawler_Db_Adapter.
     */
    const PROFILER = 'profiler';

    /**
     * Use the CASE_FOLDING constant in the config of a SpamTrawler_Db_Adapter.
     */
    const CASE_FOLDING = 'caseFolding';

    /**
     * Use the FETCH_MODE constant in the config of a SpamTrawler_Db_Adapter.
     */
    const FETCH_MODE = 'fetchMode';

    /**
     * Use the AUTO_QUOTE_IDENTIFIERS constant in the config of a SpamTrawler_Db_Adapter.
     */
    const AUTO_QUOTE_IDENTIFIERS = 'autoQuoteIdentifiers';

    /**
     * Use the ALLOW_SERIALIZATION constant in the config of a SpamTrawler_Db_Adapter.
     */
    const ALLOW_SERIALIZATION = 'allowSerialization';

    /**
     * Use the AUTO_RECONNECT_ON_UNSERIALIZE constant in the config of a SpamTrawler_Db_Adapter.
     */
    ...
    /**
     * Factory for SpamTrawler_Db_Adapter_Abstract classes.
     *
     * First argument may be a string containing the base of the adapter class
     * name, e.g. 'Mysqli' corresponds to class SpamTrawler_Db_Adapter_Mysqli.  This
     ...
     *
     * First argument may alternatively be an object of type SpamTrawler_Config.
     ...
     *
     * If the first argument is of type SpamTrawler_Config, it is assumed to contain
     * all parameters, and the second argument is ignored.
     *
     * @param  mixed $adapter String name of base adapter class, or SpamTrawler_Config object.
     * @param  mixed $config  OPTIONAL; an array or SpamTrawler_Config object with adapter parameters.
     * @return SpamTrawler_Db_Adapter_Abstract
     * @throws SpamTrawler_Db_Exception
     */
    public static function factory($adapter, $config = array())
    {
        ...
        /*
         * Convert SpamTrawler_Config argument to plain string
         * adapter name and separate config object.
         */
        if ($adapter instanceof SpamTrawler_Config) {
        ...
        if (!is_array($config)) {
            /**
             * @see SpamTrawler_Db_Exception
             */
            require_once 'SpamTrawler/Db/Exception.php';
            throw new SpamTrawler_Db_Exception('Adapter parameters must be in an array or a SpamTrawler_Config object');
        }
        ...
        if (!is_string($adapter) || empty($adapter)) {
             ...
             */
            require_once 'SpamTrawler/Db/Exception.php';
            throw new SpamTrawler_Db_Exception('Adapter name must be specified in a string');
        }

        /*
         * Form full adapter class name
         */
        $adapterNamespace = 'SpamTrawler_Db_Adapter';
        ...
        if (!class_exists($adapterName)) {
            ...
            SpamTrawler_Loader::loadClass($adapterName);
        }
         ...
         */
        if (! $dbAdapter instanceof SpamTrawler_Db_Adapter_Abstract) {
            /**
             * @see SpamTrawler_Db_Exception
             */
            require_once 'SpamTrawler/Db/Exception.php';
            throw new SpamTrawler_Db_Exception("Adapter class '$adapterName' does not extend SpamTrawler_Db_Adapter_Abstract");
        }
