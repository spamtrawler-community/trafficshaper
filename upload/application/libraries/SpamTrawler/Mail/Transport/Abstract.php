 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class SpamTrawler_Mail_Transport_Abstract
{
    /**
     * Mail body
     * @var string
    ...
    /**
     * SpamTrawler_Mail object
     * @var false|SpamTrawler_Mail
     * @access protected
     */
     ...
     * on the mail parts present in the {@link $_mail SpamTrawler_Mail object} present.
     *
        ...
        if (null !== $boundary) {
            ...
            if (!$type) {
                if ($this->_mail->hasAttachments) {
                    $type = SpamTrawler_Mime::MULTIPART_MIXED;
                } elseif ($this->_mail->getBodyText() && $this->_mail->getBodyHtml()) {
                    $type = SpamTrawler_Mime::MULTIPART_ALTERNATIVE;
                } else {
                    $type = SpamTrawler_Mime::MULTIPART_MIXED;
                }
            }
     ...
     * @access protected
     * @return void
     * @throws SpamTrawler_Mail_Transport_Exception if any header lines exceed 998
     * characters
     */
    protected function _prepareHeaders($headers)
    {
        if (!$this->_mail) {
            /**
             * @see SpamTrawler_Mail_Transport_Exception
             */
            require_once 'SpamTrawler/Mail/Transport/Exception.php';
            throw new SpamTrawler_Mail_Transport_Exception('Missing SpamTrawler_Mail object in _mail property');
        }
        ...
        if (!$sane) {
             ...
             */
            require_once 'SpamTrawler/Mail/Transport/Exception.php';
            throw new SpamTrawler_Mail_Exception('At least one mail header line is too long');
        }
    }
     ...
     * multipart/alternative SpamTrawler_Mime_Part containing the headers and contents
     * of each. Otherwise, uses whichever of the text or HTML parts present.
     *
     * The content part is then prepended to the list of SpamTrawler_Mime_Parts for
     * this message.
     *
     * @return void
     */
    protected function _buildBody()
    {
            ...
            && ($html = $this->_mail->getBodyHtml()))
        {
            ...
            $mime = new SpamTrawler_Mime(null);
            ...
            $mp->type     = SpamTrawler_Mime::MULTIPART_ALTERNATIVE;
        ...
        if (!$body) {
             ...
             */
            require_once 'SpamTrawler/Mail/Transport/Exception.php';
            throw new SpamTrawler_Mail_Transport_Exception('No body specified');
        }
        ...
        foreach ($headers as $header) {
            // Headers in SpamTrawler_Mime_Part are kept as arrays with two elements, a
            // key and a value
     ...
     *
     * @param  SpamTrawler_Mail $mail
     * @access public
     * @return void
     * @throws SpamTrawler_Mail_Transport_Exception if mail is empty
     */
    public function send(SpamTrawler_Mail $mail)
    {
        ...
        if ($count < 1) {
             ...
             */
            require_once 'SpamTrawler/Mail/Transport/Exception.php';
            throw new SpamTrawler_Mail_Transport_Exception('Empty mail cannot be sent');
        }

        if ($count > 1) {
            // Multipart message; create new MIME object and boundary
            $mime     = new SpamTrawler_Mime($this->_mail->getMimeBoundary());
        ...
        // Create message body
        // This is done so that the same SpamTrawler_Mail object can be used in
        // multiple transports
        $message = new SpamTrawler_Mime_Message();
