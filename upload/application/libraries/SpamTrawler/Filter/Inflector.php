...
/**
 * @see SpamTrawler_Loader_PluginLoader
 */
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Filter_Inflector implements SpamTrawler_Filter_Interface
{
    /**
     * @var SpamTrawler_Loader_PluginLoader_Interface
     */
    ...
    {
        if (!$this->_pluginLoader instanceof SpamTrawler_Loader_PluginLoader_Interface) {
            $this->_pluginLoader = new SpamTrawler_Loader_PluginLoader(array('SpamTrawler_Filter_' => 'SpamTrawler/Filter/'), __CLASS__);
        }
     ...
     * Set PluginLoader
     *
     * @param SpamTrawler_Loader_PluginLoader_Interface $pluginLoader
     * @return SpamTrawler_Filter_Inflector
     */
    public function setPluginLoader(SpamTrawler_Loader_PluginLoader_Interface $pluginLoader)
    {
    ...
    /**
     * Use SpamTrawler_Config object to set object state
     *
     ...
     * @param string $spec
     * @param int $index
     * @return SpamTrawler_Filter_Interface|false
     */
     ...
     *
     * @param string $spec
     * @param array|string|SpamTrawler_Filter_Interface $ruleSet
     * @return SpamTrawler_Filter_Inflector
     */
    ...
    public function filter($source)
    {
        ...
        if ($this->_throwTargetExceptionsOn && (preg_match('#(?='.$pregQuotedTargetReplacementIdentifier.'[A-Za-z]{1})#', $inflectedTarget) == true)) {
            ...
            throw new SpamTrawler_Filter_Exception('A replacement identifier ' . $this->_targetReplacementIdentifier . ' was found inside the inflected target, perhaps a rule was not satisfied with a target source?  Unsatisfied inflected target: ' . $inflectedTarget);
        }
    ...
    {
        if ($rule instanceof SpamTrawler_Filter_Interface) {
            return $rule;
        }
        ...
        if (!$ruleObject instanceof SpamTrawler_Filter_Interface) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('No class named ' . $rule . ' implementing SpamTrawler_Filter_Interface could be found');
        }
