...
class Firewall_CountryBlacklist_Controller_Manage extends SpamTrawler_BaseClasses_Modules_Controller
{
    ...
    public function get()
    {
        $oManage = new Firewall_CountryBlacklist_Model_Manage;
        $oManage->get();
    }
    ...
    public function create()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_GET['iso'])) {
                $sModels = '[{"id":"","iso":"' . $_GET['iso'] . '","comment":"","updated":""}]';
            } else {
                exit('No Data Received!');
            }
        }
        //Model constructor expects $oInput to be object
        $oManage = new Firewall_CountryBlacklist_Model_Manage(json_decode($sModels));
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            exit('Country has been Blacklisted successfully!');
        }
    }

    public function addContinent(){
        $aAllowedContinents = array('AF','AN','AS','OC','EU','NA','SA');
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['continentISO']) && in_array($_POST['continentISO'], $aAllowedContinents)) {
            $oManage = new Firewall_CountryBlacklist_Model_Manage();
            $oManage->addContinent($_POST['continentISO']);
        } else {
            exit('Invalid Continent!');
        }
    }

    public function update()
    {
        $oManage = new Firewall_CountryBlacklist_Model_Manage(json_decode($_POST['models']));
    ...
    {
        $oManage = new Firewall_CountryBlacklist_Model_Manage(NULL);
        $oManage->deleteCache();
    }
