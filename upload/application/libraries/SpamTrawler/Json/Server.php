...
/**
 * @see SpamTrawler_Server_Abstract
 */
require_once 'SpamTrawler/Server/Abstract.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Json_Server extends SpamTrawler_Server_Abstract
{
    /**#@+
     * Version Constants
     */
    ...
    /**
     * Request object
     * @var SpamTrawler_Json_Server_Request
     */
    protected $_request;

    /**
     * Response object
     * @var SpamTrawler_Json_Server_Response
     */
    protected $_response;
    ...
    /**
     * @var SpamTrawler_Server_Description
     */
    protected $_table;
     ...
     * @return SpamTrawler_Json_Server
     */
    public function addFunction($function, $namespace = '')
    {
        if (!is_string($function) && (!is_array($function) || (2 > count($function)))) {
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception('Unable to attach function; invalid');
        }

        if (!is_callable($function)) {
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception('Unable to attach function; does not exist');
        }
        ...
        require_once 'SpamTrawler/Server/Reflection.php';
        if (is_string($function)) {
            $method = SpamTrawler_Server_Reflection::reflectFunction($function, $argv, $namespace);
        } else {
            $class  = array_shift($function);
            $action = array_shift($function);
            $reflection = SpamTrawler_Server_Reflection::reflectClass($class, $argv, $namespace);
    ...
    {
        require_once 'SpamTrawler/Json/Server/Error.php';
        $error = new SpamTrawler_Json_Server_Error($fault, $code, $data);
     ...
     * Handle request
     *
     * @param  SpamTrawler_Json_Server_Request $request
     * @return null|SpamTrawler_Json_Server_Response
     */
    public function handle($request = false)
    {
        if ((false !== $request) && (!$request instanceof SpamTrawler_Json_Server_Request)) {
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception('Invalid request type provided; cannot handle');
        } elseif ($request) {
     ...
     *
     * @param  array|SpamTrawler_Server_Definition $definition
     * @return void
     */
    public function loadFunctions($definition)
    {
        if (!is_array($definition) && (!$definition instanceof SpamTrawler_Server_Definition)) {
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception('Invalid definition provided to loadFunctions()');
        }
     ...
     * @return SpamTrawler_Json_Server
     */
    public function setRequest(SpamTrawler_Json_Server_Request $request)
    {
     ...
     *
     * @return SpamTrawler_Json_Server_Request
     */
    public function getRequest()
    {
        if (null === ($request = $this->_request)) {
            require_once 'SpamTrawler/Json/Server/Request/Http.php';
            $this->setRequest(new SpamTrawler_Json_Server_Request_Http());
        }
     ...
     * Set response object
     *
     * @param  SpamTrawler_Json_Server_Response $response
     * @return SpamTrawler_Json_Server
     */
    public function setResponse(SpamTrawler_Json_Server_Response $response)
    {
    ...
    public function getResponse()
    {
        if (null === ($response = $this->_response)) {
            require_once 'SpamTrawler/Json/Server/Response/Http.php';
            $this->setResponse(new SpamTrawler_Json_Server_Response_Http());
        }
     ...
     * Retrieve SMD object
     *
     * @return SpamTrawler_Json_Server_Smd
     */
    public function getServiceMap()
    {
        if (null === $this->_serviceMap) {
            require_once 'SpamTrawler/Json/Server/Smd.php';
            $this->_serviceMap = new SpamTrawler_Json_Server_Smd();
        }
     ...
     *
     * @param  SpamTrawler_Server_Reflection_Function $method
     * @return void
     */
    protected function _addMethodServiceMap(SpamTrawler_Server_Method_Definition $method)
    {
        $serviceInfo = array(
            'name'   => $method->getName(),
     ...
     *
     * @param  SpamTrawler_Server_Reflection_Function_Abstract $method
     * @return string|array
     */
    protected function _getParams(SpamTrawler_Server_Method_Definition $method)
    {
        $params = array();
     ...
     * @return string|array
     */
    protected function _getReturnType(SpamTrawler_Server_Method_Definition $method)
    {
        $return = array();
        foreach ($method->getPrototypes() as $prototype) {
    ...
    protected function _getSmdMethods()
    {
        if (null === $this->_smdMethods) {
            ...
            require_once 'SpamTrawler/Json/Server/Smd.php';
            $methods = get_class_methods('SpamTrawler_Json_Server_Smd');
        ...
        if ( is_string( key( $params ) ) ) {
            ...
            foreach( $reflection->getParameters() as $refParam ) {
                if( isset( $params[ $refParam->getName() ] ) ) {
                    ...
                    throw new SpamTrawler_Server_Exception(
