 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Filter_Compress_Zip extends SpamTrawler_Filter_Compress_CompressAbstract
{
    /**
     * Compression Options
     * array(
    ...
    public function __construct($options = null)
    {
        if (!extension_loaded('zip')) {
            ...
            throw new SpamTrawler_Filter_Exception('This filter needs the zip extension');
        }
    ...
    public function compress($content)
    {
        ...
        if ($res !== true) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception($this->_errorString($res));
        }
    ...
    public function decompress($content)
    {
        ...
        if (file_exists($content)) {
            ...
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('ZIP Archive not found');
        }
        ...
        if (empty($target) || !is_dir($target)) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('No target for ZIP decompression set');
        }
        ...
        if (version_compare(PHP_VERSION, '5.2.8', '<')) {
            for ($i = 0; $i < $zip->numFiles; $i++) {
                ...
                {
                    require_once 'SpamTrawler/Filter/Exception.php';
                    throw new SpamTrawler_Filter_Exception('Upward directory traversal was detected inside ' . $archive
