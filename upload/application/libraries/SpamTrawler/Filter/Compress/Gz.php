 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Filter_Compress_Gz extends SpamTrawler_Filter_Compress_CompressAbstract
{
    /**
     * Compression Options
     * array(
     ...
     * Class constructor
     *
     * @param array|SpamTrawler_Config|null $options (Optional) Options to set
     */
    public function __construct($options = null)
    {
        if (!extension_loaded('zlib')) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('This filter needs the zlib extension');
        }
     ...
     *
     * @param integer $level
     * @return SpamTrawler_Filter_Compress_Gz
     */
    public function setLevel($level)
    {
        if (($level < 0) || ($level > 9)) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('Level must be between 0 and 9');
        }
    ...
    public function setMode($mode)
    {
        if (($mode != 'compress') && ($mode != 'deflate')) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('Given compression mode not supported');
        }
    ...
    public function compress($content)
    {
        ...
        if (!empty($archive)) {
            ...
            if (!$file) {
                require_once 'SpamTrawler/Filter/Exception.php';
                throw new SpamTrawler_Filter_Exception("Error opening the archive '" . $this->_options['archive'] . "'");
            }
    ...
    public function decompress($content)
    {
        ...
        if (@file_exists($archive)) {
            ...
            if (!$handler) {
                require_once 'SpamTrawler/Filter/Exception.php';
                throw new SpamTrawler_Filter_Exception("Error opening the archive '" . $archive . "'");
            }
