...
<div class="row">
    <div class="small-4 columns">
        <input type="text" id="query_organizationblacklist" class="radius" value="" placeholder="Search">
    </div>
    <div class="small-2 columns">
        <select id="column_organizationblacklist" class="radius">
            <option value="organization">Organization</option>
            <option value="comment">Comment</option>
        </select>
    </div>
    <div class="small-2 columns">
        <select id="operator_organizationblacklist" class="radius">
    ...
    <div class="small-2 columns">
        <input type="button" id="btnFilter_organizationblacklist" class="k-button" value="Filter">
        <input type="button" id="btnReset_organizationblacklist" class="k-button" value="Reset">
    </div>
</div>

        <div id="gridOrganizationBlacklist"></div>
        {literal}
        <script>
            $(document).ready(function () {
                var crudServiceBaseUrl = "{/literal}{$requestprotocol}://{$linkurl}Firewall/OrganizationBlacklist/Manage{literal}",
                ...
                $("#gridOrganizationBlacklist").kendoGrid({
                    ...
                    detailTemplate: kendo.template($("#commentTemplateOrganizationBlacklist").html()),
                    toolbar: ["create"],
                    columns: [
                        {
                            field: "organization",
                ...
                $("#btnFilter_organizationblacklist").on("click", function(e) {
                    var ds = $("#gridOrganizationBlacklist").data("kendoGrid").dataSource;
                    ds.filter([{
                        "field": $("#column_organizationblacklist").val(),
                        "operator": $("#operator_organizationblacklist").val(),
                        "value": $("#query_organizationblacklist").val()}
                    ]);
                });

                $("#btnReset_organizationblacklist").on("click", function(e) {
                    $("#query_organizationblacklist").val('');
                    var ds = $("#gridOrganizationBlacklist").data("kendoGrid").dataSource;
                    ds.filter([]);
                });
            });
...
<!-- popup editor template -->
    <script id="popup_editorOrganizationBlacklist" type="text/x-kendo-template">
       <div id="errors">Shows Errors here!</div>
    </script>

        <script type="text/x-kendo-template" id="commentTemplateOrganizationBlacklist">
            <div class="row">
                <div>Comment:</div>
