...
/**
 * @see SpamTrawler_Db_Expr
 */
require_once 'SpamTrawler/Db/Expr.php';
...
class SpamTrawler_Db_Select
{
     ...
     * Class constructor
     *
     * @param SpamTrawler_Db_Adapter_Abstract $adapter
     */
    public function __construct(SpamTrawler_Db_Adapter_Abstract $adapter)
    {
     ...
     * table.
     *
     * The second parameter can be a single string or SpamTrawler_Db_Expr object,
     * or else an array of strings or SpamTrawler_Db_Expr objects.
     *
     * The first parameter can be null or an empty string, in which case
     * no correlation name is generated or prepended to the columns named
     * in the second parameter.
     *
     * @param  array|string|SpamTrawler_Db_Expr $name The table name or an associative array
     ...
     *
     * The parameter can be a single string or SpamTrawler_Db_Expr object,
     * or else an array of strings or SpamTrawler_Db_Expr objects.
     *
     * @param  array|string|SpamTrawler_Db_Expr $cols The columns to select from this table.
    ...
    public function columns($cols = '*', $correlationName = null)
    {
        ...
        if (!array_key_exists($correlationName, $this->_parts[self::FROM])) {
            /**
             * @see SpamTrawler_Db_Select_Exception
             */
            require_once 'SpamTrawler/Db/Select/Exception.php';
            throw new SpamTrawler_Db_Select_Exception("No table has been specified for the FROM clause");
        }
     ...
     *
     * The first parameter has to be an array of SpamTrawler_Db_Select or
     * sql query strings.
     *
     * <code>
     * $sql1 = $db->select();
    ...
    public function union($select = array(), $type = self::SQL_UNION)
    {
        if (!is_array($select)) {
            require_once 'SpamTrawler/Db/Select/Exception.php';
            throw new SpamTrawler_Db_Select_Exception(
                "union() only accepts an array of SpamTrawler_Db_Select instances of sql query strings."
            );
        }

        if (!in_array($type, self::$_unionTypes)) {
            require_once 'SpamTrawler/Db/Select/Exception.php';
            throw new SpamTrawler_Db_Select_Exception("Invalid union type '{$type}'");
        }
     ...
     *
     * @param  array|string|SpamTrawler_Db_Expr $name The table name.
    ...
    public function group($spec)
    {
        ...
        foreach ($spec as $val) {
            if (preg_match('/\(.*\)/', (string) $val)) {
                $val = new SpamTrawler_Db_Expr($val);
            }
    ...
    public function order($spec)
    {
        ...
        foreach ($spec as $val) {
            if ($val instanceof SpamTrawler_Db_Expr) {
                $expr = $val->__toString();
                if (empty($expr)) {
                    continue;
                }
     ...
     * @param string $part
     * @return mixed
     * @throws SpamTrawler_Db_Select_Exception
     */
    public function getPart($part)
    {
        ...
        if (!array_key_exists($part, $this->_parts)) {
            require_once 'SpamTrawler/Db/Select/Exception.php';
            throw new SpamTrawler_Db_Select_Exception("Invalid Select part '$part'");
        }
     ...
     * @return PDO_Statement|SpamTrawler_Db_Statement
     */
    ...
    /**
     * Gets the SpamTrawler_Db_Adapter_Abstract for this
     * particular SpamTrawler_Db_Select object.
     *
     ...
     * @param  array|string|SpamTrawler_Db_Expr $name Table name
     * @param  string $cond Join on this condition
     * @param  array|string $cols The columns to select from the joined table
     * @param  string $schema The database name to specify, if any.
     * @return SpamTrawler_Db_Select This SpamTrawler_Db_Select object
    ...
    protected function _join($type, $name, $cond, $cols, $schema = null)
    {
        if (!in_array($type, self::$_joinTypes) && $type != self::FROM) {
             ...
             */
            require_once 'SpamTrawler/Db/Select/Exception.php';
            throw new SpamTrawler_Db_Select_Exception("Invalid join type '$type'");
        }

        if (count($this->_parts[self::UNION])) {
            require_once 'SpamTrawler/Db/Select/Exception.php';
            throw new SpamTrawler_Db_Select_Exception("Invalid use of table with " . self::SQL_UNION);
        }
        ...
        } elseif ($name instanceof SpamTrawler_Db_Expr|| $name instanceof SpamTrawler_Db_Select) {
            $tableName = $name;
        ...
        if (!empty($correlationName)) {
            if (array_key_exists($correlationName, $this->_parts[self::FROM])) {
                 ...
                 */
                require_once 'SpamTrawler/Db/Select/Exception.php';
                throw new SpamTrawler_Db_Select_Exception("You cannot define a correlation name '$correlationName' more than once");
            }
    ...
    public function _joinUsing($type, $name, $cond, $cols = '*', $schema = null)
    {
        if (empty($this->_parts[self::FROM])) {
            require_once 'SpamTrawler/Db/Select/Exception.php';
            throw new SpamTrawler_Db_Select_Exception("You can only perform a joinUsing after specifying a FROM table");
        }
    ...
    protected function _tableCols($correlationName, $cols, $afterCorrelationName = null)
    {
        ...
        foreach (array_filter($cols) as $alias => $col) {
            ...
            if (is_string($col)) {
                ...
                // Check for columns that look like functions and convert to SpamTrawler_Db_Expr
                if (preg_match('/\(.*\)/', $col)) {
                    $col = new SpamTrawler_Db_Expr($col);
    ...
    protected function _where($condition, $value = null, $type = null, $bool = true)
    {
        if (count($this->_parts[self::UNION])) {
            require_once 'SpamTrawler/Db/Select/Exception.php';
            throw new SpamTrawler_Db_Select_Exception("Invalid use of where clause with " . self::SQL_UNION);
        }
    ...
    protected function _renderColumns($sql)
    {
        ...
        foreach ($this->_parts[self::COLUMNS] as $columnEntry) {
            ...
            if ($column instanceof SpamTrawler_Db_Expr) {
                $columns[] = $this->_adapter->quoteColumnAs($column, $alias, true);
            } else {
                if ($column == self::SQL_WILDCARD) {
                    $column = new SpamTrawler_Db_Expr(self::SQL_WILDCARD);
                    $alias = null;
                }
                if (empty($correlationName)) {
                    $columns[] = $this->_adapter->quoteColumnAs($column, $alias, true);
    ...
    protected function _renderUnion($sql)
    {
        if ($this->_parts[self::UNION]) {
            ...
            foreach ($this->_parts[self::UNION] as $cnt => $union) {
                ...
                if ($target instanceof SpamTrawler_Db_Select) {
     ...
     * @return SpamTrawler_Db_Select
     * @throws SpamTrawler_Db_Select_Exception If an invalid method is called.
     */
        ...
        if (preg_match('/^join([a-zA-Z]*?)Using$/', $method, $matches)) {
            ...
            if ($type) {
                ...
                if (!in_array($type, self::$_joinTypes)) {
                    require_once 'SpamTrawler/Db/Select/Exception.php';
                    throw new SpamTrawler_Db_Select_Exception("Unrecognized method '$method()'");
                }
                if (in_array($type, array(self::CROSS_JOIN, self::NATURAL_JOIN))) {
                    require_once 'SpamTrawler/Db/Select/Exception.php';
                    throw new SpamTrawler_Db_Select_Exception("Cannot perform a joinUsing with method '$method()'");
                }
            } else {
