...
/**
 * @see SpamTrawler_Config
 */
require_once 'SpamTrawler/Config.php';

/** @see SpamTrawler_Xml_Security */
require_once 'SpamTrawler/Xml/Security.php';

/** @see SpamTrawler_Xml_Exception */
require_once 'SpamTrawler/Xml/Exception.php';

/**
 * XML Adapter for SpamTrawler_Config
 *
 * @category  SpamTrawler
 * @package   SpamTrawler_Config
 * @copyright  Copyright (c) 2005-2014 SpamTrawler Technologies USA Inc. (http://www.zend.com)
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Config_Xml extends SpamTrawler_Config
{
    /**
     ...
     * SpamTrawler_Config. If provided as an array, there are two configuration
     ...
     * @throws SpamTrawler_Config_Exception When xml is not set or cannot be loaded
     * @throws SpamTrawler_Config_Exception When section $sectionName cannot be found in $xml
     */
    public function __construct($xml, $section = null, $options = false)
    {
        if (empty($xml)) {
            require_once 'SpamTrawler/Config/Exception.php';
            throw new SpamTrawler_Config_Exception('Filename is not set');
        }
        ...
        if (strstr($xml, '<?xml')) {
            $config = SpamTrawler_Xml_Security::scan($xml);
        } else {
            try {
                if (!$config = SpamTrawler_Xml_Security::scanFile($xml)) {
                    require_once 'SpamTrawler/Config/Exception.php';
                    throw new SpamTrawler_Config_Exception(
                        "Error failed to load $xml file"
                    );
                }
            } catch (SpamTrawler_Xml_Exception $e) {
                require_once 'SpamTrawler/Config/Exception.php';
                throw new SpamTrawler_Config_Exception(
                    $e->getMessage()
                );
        ...
        if ($this->_loadFileErrorStr !== null) {
            require_once 'SpamTrawler/Config/Exception.php';
            throw new SpamTrawler_Config_Exception($this->_loadFileErrorStr);
        }
        ...
        if ($section === null) {
            ...
            foreach ($section as $sectionName) {
                if (!isset($config->$sectionName)) {
                    require_once 'SpamTrawler/Config/Exception.php';
                    throw new SpamTrawler_Config_Exception("Section '$sectionName' cannot be found in $xml");
                }
            ...
            if (!isset($config->$section)) {
                require_once 'SpamTrawler/Config/Exception.php';
                throw new SpamTrawler_Config_Exception("Section '$section' cannot be found in $xml");
            }
     ...
     * @throws SpamTrawler_Config_Exception When $section cannot be found
     * @return array
     */
    protected function _processExtends(SimpleXMLElement $element, $section, array $config = array())
    {
        if (!isset($element->$section)) {
            require_once 'SpamTrawler/Config/Exception.php';
            throw new SpamTrawler_Config_Exception("Section '$section' cannot be found");
        }
    ...
    protected function _toArray(SimpleXMLElement $xmlObject)
    {
        ...
        if (count($xmlObject->children(self::XML_NAMESPACE)) > 0) {
            if (count($xmlObject->children()) > 0) {
                require_once 'SpamTrawler/Config/Exception.php';
                throw new SpamTrawler_Config_Exception("A node with a 'const' childnode may not have any other children");
            }
                ...
                switch ($node->localName) {
                    case 'const':
                        if (!$node->hasAttributeNS(self::XML_NAMESPACE, 'name')) {
                            require_once 'SpamTrawler/Config/Exception.php';
                            throw new SpamTrawler_Config_Exception("Misssing 'name' attribute in 'const' node");
                        }
                        ...
                        if (!defined($constantName)) {
                            require_once 'SpamTrawler/Config/Exception.php';
                            throw new SpamTrawler_Config_Exception("Constant with name '$constantName' was not defined");
                        }
                    ...
                    default:
                        require_once 'SpamTrawler/Config/Exception.php';
                        throw new SpamTrawler_Config_Exception("Unknown node with name '$node->localName' found");
                }
            }
