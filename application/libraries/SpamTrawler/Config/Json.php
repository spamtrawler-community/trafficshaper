...
/**
 * JSON Adapter for SpamTrawler_Config
 *
 * @category  SpamTrawler
 * @package   SpamTrawler_Config
 * @copyright Copyright (c) 2005-2009 SpamTrawler Technologies USA Inc. (http://www.zend.com)
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Config_Json extends SpamTrawler_Config
{
    /**
     ...
     * @throws SpamTrawler_Config_Exception When JSON text is not set or cannot be loaded
     * @throws SpamTrawler_Config_Exception When section $sectionName cannot be found in $json
     */
    public function __construct($json, $section = null, $options = false)
    {
        ...
        // Parse/decode
        try {
            $config = SpamTrawler_Json::decode($json);
        } catch (SpamTrawler_Json_Exception $e) {
            // decode failed
            require_once 'SpamTrawler/Config/Exception.php';
            throw new SpamTrawler_Config_Exception("Error parsing JSON data");
        }
        ...
        if ($section === null) {
            ...
            foreach ($section as $sectionName) {
                if (!isset($config[$sectionName])) {
                    require_once 'SpamTrawler/Config/Exception.php';
                    throw new SpamTrawler_Config_Exception(sprintf('Section "%s" cannot be found', $sectionName));
                }
            ...
            if (!isset($config[$section])) {
                require_once 'SpamTrawler/Config/Exception.php';
                throw new SpamTrawler_Config_Exception(sprintf('Section "%s" cannot be found', $section));
            }
    ...
    protected function _processExtends(array $data, $section, array $config = array())
    {
        ...
        if (is_array($thisSection) && isset($thisSection[self::EXTENDS_NAME])) {
            if (is_array($thisSection[self::EXTENDS_NAME])) {
                require_once 'SpamTrawler/Config/Exception.php';
                throw new SpamTrawler_Config_Exception('Invalid extends clause: must be a string; array received');
            }
