...
/**
 * @see SpamTrawler_Json_Server_Smd
 */
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Json_Server_Smd_Service
{
    /**#@+
     * Service metadata
     * @var string
     */
    protected $_envelope  = SpamTrawler_Json_Server_Smd::ENV_JSONRPC_1;
    protected $_name;
    protected $_return;
    protected $_target;
    ...
    protected $_envelopeTypes = array(
        SpamTrawler_Json_Server_Smd::ENV_JSONRPC_1,
        SpamTrawler_Json_Server_Smd::ENV_JSONRPC_2,
    );
     ...
     * @return void
     * @throws SpamTrawler_Json_Server_Exception if no name provided
     */
    public function __construct($spec)
    {
        ...
        if (null == $this->getName()) {
            ...
            throw new SpamTrawler_Json_Server_Exception('SMD service description requires a name; none provided');
        }
    }
     ...
     *
     * @param  array $options
     * @return SpamTrawler_Json_Server_Smd_Service
     */
     ...
     * @param  string $name
     * @return SpamTrawler_Json_Server_Smd_Service
     * @throws SpamTrawler_Json_Server_Exception
     */
    public function setName($name)
    {
        ...
        if (!preg_match($this->_nameRegex, $name)) {
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception(sprintf('Invalid name "%s" provided for service; must follow PHP method naming conventions', $name));
        }
        $this->_name = $name;
        return $this;
    }
    ...
    public function setTransport($transport)
    {
        if (!in_array($transport, $this->_transportTypes)) {
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception(sprintf('Invalid transport "%s"; please select one of (%s)', $transport, implode(', ', $this->_transportTypes)));
        }
    ...
    public function setEnvelope($envelopeType)
    {
        if (!in_array($envelopeType, $this->_envelopeTypes)) {
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception(sprintf('Invalid envelope type "%s"; please specify one of (%s)', $envelopeType, implode(', ', $this->_envelopeTypes)));
        }
    ...
    public function addParam($type, array $options = array(), $order = null)
    {
        if (is_string($type)) {
            ...
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception('Invalid param type provided');
        }
    ...
    public function setReturn($type)
    {
        if (is_string($type)) {
            ...
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception('Invalid param type provided ("' . gettype($type) .'")');
        }
        $this->_return = $type;
        return $this;
    }
    ...
    public function toJson()
    {
        ...
        return SpamTrawler_Json::encode($service);
    }
    ...
    protected function _validateParamType($type, $isReturn = false)
    {
        if (!is_string($type)) {
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception('Invalid param type provided ("' . $type .'")');
        }
