...
/**
 * @see SpamTrawler_Config_Json
 */
require_once 'SpamTrawler/Config/Json.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Config_Writer_Json extends SpamTrawler_Config_Writer_FileAbstract
{
    /**
     ...
     * @return SpamTrawler_Config_Writer_Json
     */
    ...
    /**
     * Render a SpamTrawler_Config into a JSON config string.
     *
     * @since 1.10
     * @return string
     */
    public function render()
    {
        ...
        foreach ($extends as $section => $parentSection) {
            $data[$section][SpamTrawler_Config_Json::EXTENDS_NAME] = $parentSection;
        }

        // Ensure that each "extends" section actually exists
        foreach ($data as $section => $sectionData) {
            if (is_array($sectionData) && isset($sectionData[SpamTrawler_Config_Json::EXTENDS_NAME])) {
                $sectionExtends = $sectionData[SpamTrawler_Config_Json::EXTENDS_NAME];
                if (!isset($data[$sectionExtends])) {
                    // Remove "extends" declaration if section does not exist
                    unset($data[$section][SpamTrawler_Config_Json::EXTENDS_NAME]);
                }
            }
        }

        $out = SpamTrawler_Json::encode($data);
        if ($this->prettyPrint()) {
             $out = SpamTrawler_Json::prettyPrint($out);
        }
        return $out;
    }
}
