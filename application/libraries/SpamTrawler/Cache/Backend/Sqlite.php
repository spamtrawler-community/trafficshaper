 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Cache_Backend_Sqlite extends SpamTrawler_Cache_Backend implements SpamTrawler_Cache_Backend_ExtendedInterface
{
    /**
     * Available options
     *
    ...
    public function __construct(array $options = array())
    {
        ...
        if ($this->_options['cache_db_complete_path'] === null) {
            SpamTrawler_Cache::throwException('cache_db_complete_path option has to set');
        }
        if (!extension_loaded('sqlite')) {
            SpamTrawler_Cache::throwException("Cannot use SQLite storage because the 'sqlite' extension is not loaded in the current PHP environment");
        }
        $this->_getConnection();
    }
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        $res = $this->_query($sql);
        if (!$res) {
            $this->_log("SpamTrawler_Cache_Backend_Sqlite::save() : impossible to store the cache id=$id");
            return false;
        }
        $res = true;
        foreach ($tags as $tag) {
    ...
    private function _getConnection()
    {
        if (is_resource($this->_db)) {
            ...
            if (!(is_resource($this->_db))) {
                SpamTrawler_Cache::throwException("Impossible to open " . $this->_options['cache_db_complete_path'] . " cache DB file");
            }
            return $this->_db;
        }
    }
    ...
    private function _registerTag($id, $tag) {
        ...
        if (!$res) {
            $this->_log("SpamTrawler_Cache_Backend_Sqlite::_registerTag() : impossible to register tag=$tag on id=$id");
            return false;
        }
        return true;
    }
    ...
    private function _checkStructureVersion()
    {
        ...
        if (((int) $row['num']) != 1) {
            // old cache structure
            $this->_log('SpamTrawler_Cache_Backend_Sqlite::_checkStructureVersion() : old cache structure version detected => the cache is going to be dropped');
            return false;
        }
        return true;
    }
     ...
     * @return boolean True if no problem
     */
    private function _clean($mode = SpamTrawler_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        switch ($mode) {
    ...
    private function _checkAndBuildStructure()
    {
        if (!($this->_structureChecked)) {
            if (!$this->_checkStructureVersion()) {
                ...
                if (!$this->_checkStructureVersion()) {
                    SpamTrawler_Cache::throwException("Impossible to build cache structure in " . $this->_options['cache_db_complete_path']);
                }
            }
