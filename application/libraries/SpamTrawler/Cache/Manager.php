...
/** @see SpamTrawler_Cache_Exception */
require_once 'SpamTrawler/Cache/Exception.php';

/** @see SpamTrawler_Cache */
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Cache_Manager
{
    /**
     ...
     *
     * @param  string $name
     * @param  SpamTrawler_Cache_Core $cache
     * @return SpamTrawler_Cache_Manager
     */
    public function setCache($name, SpamTrawler_Cache_Core $cache)
    {
    ...
    public function getCache($name)
    {
        ...
        if (isset($this->_optionTemplates[$name])) {
            if ($name == self::PAGECACHE
                ...
                || !$this->_optionTemplates[$name]['backend']['options']['tag_cache'] instanceof SpamTrawler_Cache_Core)
            ) {
                $this->_optionTemplates[$name]['backend']['options']['tag_cache']
                    = $this->getCache(self::PAGETAGCACHE);
            }

            $this->_caches[$name] = SpamTrawler_Cache::factory(
    ...
    public function setCacheTemplate($name, $options)
    {
        if ($options instanceof SpamTrawler_Config) {
            ...
            require_once 'SpamTrawler/Cache/Exception.php';
            throw new SpamTrawler_Cache_Exception('Options passed must be in'
                . ' an associative array or instance of SpamTrawler_Config');
        }
     ...
     * @param  array $options
     * @return SpamTrawler_Cache_Manager
     * @throws SpamTrawler_Cache_Exception for invalid options format or if option templates do not have $name
     */
    public function setTemplateOptions($name, $options)
    {
        ...
        if (!isset($this->_optionTemplates[$name])) {
            throw new SpamTrawler_Cache_Exception('A cache configuration template'
