...
/**
 * @see SpamTrawler_Mail_Storage_Mbox
 */
require_once 'SpamTrawler/Mail/Storage/Mbox.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Mail_Storage_Folder_Mbox extends SpamTrawler_Mail_Storage_Mbox implements SpamTrawler_Mail_Storage_Folder_Interface
{
    /**
     * SpamTrawler_Mail_Storage_Folder root folder for folder structure
     * @var SpamTrawler_Mail_Storage_Folder
     */
     ...
     *   - filename use SpamTrawler_Mail_Storage_Mbox for a single file
    ...
    public function __construct($params)
    {
        ...
        if (isset($params->filename)) {
            ...
            throw new SpamTrawler_Mail_Storage_Exception('use SpamTrawler_Mail_Storage_Mbox for a single file');
        }
        ...
        if (!isset($params->dirname) || !is_dir($params->dirname)) {
             ...
             */
            require_once 'SpamTrawler/Mail/Storage/Exception.php';
            throw new SpamTrawler_Mail_Storage_Exception('no valid dirname given in params');
        }
     ...
     *
     * Result is save in SpamTrawler_Mail_Storage_Folder instances with the root in $this->_rootFolder.
     * $parentFolder and $parentGlobalName are only used internally for recursion.
     *
     * @param string $currentDir call with root dir, also used for recursion.
     * @param SpamTrawler_Mail_Storage_Folder|null $parentFolder used for recursion
    ...
    {
        if (!$parentFolder) {
            $this->_rootFolder = new SpamTrawler_Mail_Storage_Folder('/', '/', false);
        ...
        if (!$dh) {
             ...
             */
            require_once 'SpamTrawler/Mail/Storage/Exception.php';
            throw new SpamTrawler_Mail_Storage_Exception("can't read dir $currentDir");
        }
        while (($entry = readdir($dh)) !== false) {
            ...
            if (is_file($absoluteEntry) && $this->_isMboxFile($absoluteEntry)) {
                $parentFolder->$entry = new SpamTrawler_Mail_Storage_Folder($entry, $globalName);
                continue;
            }
            if (!is_dir($absoluteEntry) /* || $entry == '.' || $entry == '..' */) {
                continue;
            }
            $folder = new SpamTrawler_Mail_Storage_Folder($entry, $globalName, false);
    ...
    public function getFolders($rootFolder = null)
    {
        ...
        if ($currentFolder->getGlobalName() != DIRECTORY_SEPARATOR . trim($rootFolder, DIRECTORY_SEPARATOR)) {
             ...
             */
            require_once 'SpamTrawler/Mail/Storage/Exception.php';
            throw new SpamTrawler_Mail_Storage_Exception("folder $rootFolder not found");
        }
    ...
    public function selectFolder($globalName)
    {
        ...
        try {
            ...
            if (!$folder->isSelectable()) {
                 ...
                 */
                require_once 'SpamTrawler/Mail/Storage/Exception.php';
                throw new SpamTrawler_Mail_Storage_Exception("{$this->_currentFolder} is not selectable", 0, $e);
            }
             ...
             */
            require_once 'SpamTrawler/Mail/Storage/Exception.php';
            throw new SpamTrawler_Mail_Storage_Exception('seems like the mbox file has vanished, I\'ve rebuild the ' .
