 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Json_Server_Smd
{
    ...
    public function setTransport($transport)
    {
        if (!in_array($transport, $this->_transportTypes)) {
            ...
            throw new SpamTrawler_Json_Server_Exception(sprintf('Invalid transport "%s" specified', $transport));
        }
    ...
    public function setEnvelope($envelopeType)
    {
        if (!in_array($envelopeType, $this->_envelopeTypes)) {
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception(sprintf('Invalid envelope type "%s"', $envelopeType));
        }
    ...
    public function setContentType($type)
    {
        if (!preg_match($this->_contentTypeRegex, $type)) {
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception(sprintf('Invalid content type "%s" specified', $type));
        }
     ...
     * Add Service
     *
     * @param SpamTrawler_Json_Server_Smd_Service|array $service
     * @return void
     */
    public function addService($service)
    {
        require_once 'SpamTrawler/Json/Server/Smd/Service.php';

        if ($service instanceof SpamTrawler_Json_Server_Smd_Service) {
            $name = $service->getName();
        } elseif (is_array($service)) {
            $service = new SpamTrawler_Json_Server_Smd_Service($service);
            $name = $service->getName();
        } else {
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception('Invalid service passed to addService()');
        }

        if (array_key_exists($name, $this->_services)) {
            require_once 'SpamTrawler/Json/Server/Exception.php';
            throw new SpamTrawler_Json_Server_Exception('Attempt to register a service already registered detected');
        }
     ...
     *
     * @param  string $name
     * @return false|SpamTrawler_Json_Server_Smd_Service
     */
    ...
    public function toJson()
    {
        ...
        return SpamTrawler_Json::encode($this->toArray());
    }
