 ...
 *
 */
class Firewall_StopForumSpam_Helper_API {
    private $AllowTor = false;
    private $APIUrl = "http://api.stopforumspam.org/api?f=json";
    private $APIKey;
    public $iTimeout = 10;

    private function GetAPIResult($sUrl){
        //Curl
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $sUrl);
        curl_setopt($ch, CURLOPT_USERAGENT, "Registration SpamCheck " . urlencode(iconv('GBK', 'UTF-8', $_SERVER['HTTP_HOST'])));
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, $this->iTimeout);
    ...
    private function APICheckIP($IP) {
        ...
        $json = $this->GetAPIResult($FullAPIURL);
