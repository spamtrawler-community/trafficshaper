 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Cache_Frontend_Page extends SpamTrawler_Cache_Core
{
    /**
    ...
    public function __construct(array $options = array())
    {
        ...
        if (isset($this->_specificOptions['http_conditional'])) {
            if ($this->_specificOptions['http_conditional']) {
                SpamTrawler_Cache::throwException('http_conditional is not implemented for the moment !');
            }
        }
    ...
    {
        if (!is_array($options)) {
            SpamTrawler_Cache::throwException('default_options must be an array !');
        }
        foreach ($options as $key=>$value) {
            if (!is_string($key)) {
                SpamTrawler_Cache::throwException("invalid option [$key] !");
            }
            $key = strtolower($key);
    ...
    {
        if (!is_array($regexps)) {
            SpamTrawler_Cache::throwException('regexps option must be an array !');
        }
        foreach ($regexps as $regexp=>$conf) {
            if (!is_array($conf)) {
                SpamTrawler_Cache::throwException('regexps option must be an array of arrays !');
            }
            $validKeys = array_keys($this->_specificOptions['default_options']);
            foreach ($conf as $key=>$value) {
                if (!is_string($key)) {
                    SpamTrawler_Cache::throwException("unknown option [$key] !");
                }
                $key = strtolower($key);
