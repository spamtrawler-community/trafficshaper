 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Validate_Date extends SpamTrawler_Validate_Abstract
{
    ...
    protected $_messageTemplates = array(
        self::INVALID        => "Invalid type given. String, integer, array or SpamTrawler_Date expected",
     ...
     * Optional locale
     *
     * @var string|SpamTrawler_Locale|null
     */
    protected $_locale;
     ...
     *
     * @param string|array|SpamTrawler_Config $options OPTIONAL
     */
    public function __construct($options = array())
    {
        ...
        if (!array_key_exists('locale', $options)) {
            ...
            if (SpamTrawler_Registry::isRegistered('SpamTrawler_Locale')) {
                $options['locale'] = SpamTrawler_Registry::get('SpamTrawler_Locale');
            }
        }
     ...
     *
     * @return string|SpamTrawler_Locale|null
     */
     ...
     * @return SpamTrawler_Validate_Date provides a fluent interface
     */
     ...
     * according to SpamTrawler_Date, see SpamTrawler_Date::isDate()
     *
     * @param  string|array|SpamTrawler_Date $value
     * @return boolean
     */
    public function isValid($value)
    {
        if (!is_string($value) && !is_int($value) && !is_float($value) &&
            !is_array($value) && !($value instanceof SpamTrawler_Date)) {
        ...
        if (($this->_format !== null) || ($this->_locale !== null) || is_array($value) ||
             $value instanceof SpamTrawler_Date) {
            require_once 'SpamTrawler/Date.php';
            if (!SpamTrawler_Date::isDate($value, $this->_format, $this->_locale)) {
    ...
    private function _checkFormat($value)
    {
        try {
            ...
            $parsed = SpamTrawler_Locale_Format::getDate($value, array(
            ...
            if (isset($parsed['year']) and ((strpos(strtoupper($this->_format), 'YY') !== false) and
                ...
                $parsed['year'] = SpamTrawler_Date::getFullYear($parsed['year']);
            }
        } catch (Exception $e) {
