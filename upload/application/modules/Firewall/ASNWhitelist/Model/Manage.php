...
class Firewall_ASNWhitelist_Model_Manage extends Firewall_Abstract_Model_Manage
{
    protected $dbTableName = 'whitelist_asn';
    ...
    public function validate()
    {
        ...
        if(isset($this->oInput[0]->isregex) && $this->oInput[0]->isregex == 1){
    ...
    public static function validateSettings($aSettings){
        ...
        if(!ctype_digit($aSettings['conf_status'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'ASN Whitelist Status Invalid!';
        }

        /*
        if(!ctype_digit($aSettings['conf_order'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'ASN Whitelist Order Invalid!';
        }
        */
