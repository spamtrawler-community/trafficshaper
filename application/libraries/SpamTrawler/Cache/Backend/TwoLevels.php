...
/**
 * @see SpamTrawler_Cache_Backend_ExtendedInterface
 */
require_once 'SpamTrawler/Cache/Backend/ExtendedInterface.php';

/**
 * @see SpamTrawler_Cache_Backend
 */
...
class SpamTrawler_Cache_Backend_TwoLevels extends SpamTrawler_Cache_Backend implements SpamTrawler_Cache_Backend_ExtendedInterface
{
    /**
     * Available options
     *
     * =====> (string) slow_backend :
     * - Slow backend name
     * - Must implement the SpamTrawler_Cache_Backend_ExtendedInterface
     ...
     * - See SpamTrawler_Cache::factory() method
     *
     ...
     * Slow Backend
     *
     * @var SpamTrawler_Cache_Backend_ExtendedInterface
     */
    ...
    public function __construct(array $options = array())
    {
        ...
        if ($this->_options['slow_backend'] === null) {
            SpamTrawler_Cache::throwException('slow_backend option has to set');
        } elseif ($this->_options['slow_backend'] instanceof SpamTrawler_Cache_Backend_ExtendedInterface) {
            $this->_slowBackend = $this->_options['slow_backend'];
        } else {
            $this->_slowBackend = SpamTrawler_Cache::_makeBackend(
            ...
            if (!in_array('SpamTrawler_Cache_Backend_ExtendedInterface', class_implements($this->_slowBackend))) {
                SpamTrawler_Cache::throwException('slow_backend must implement the SpamTrawler_Cache_Backend_ExtendedInterface interface');
            }
        }

        if ($this->_options['fast_backend'] === null) {
            SpamTrawler_Cache::throwException('fast_backend option has to set');
        } elseif ($this->_options['fast_backend'] instanceof SpamTrawler_Cache_Backend_ExtendedInterface) {
            $this->_fastBackend = $this->_options['fast_backend'];
        } else {
            $this->_fastBackend = SpamTrawler_Cache::_makeBackend(
            ...
            if (!in_array('SpamTrawler_Cache_Backend_ExtendedInterface', class_implements($this->_fastBackend))) {
                SpamTrawler_Cache::throwException('fast_backend must implement the SpamTrawler_Cache_Backend_ExtendedInterface interface');
            }
        }
     ...
     * SpamTrawler_Cache::CLEANING_MODE_NOT_MATCHING_TAG => remove cache entries not {matching one of the given tags}
     *                                               ($tags can be an array of strings or a single string)
    ...
    public function clean($mode = SpamTrawler_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        switch($mode) {
            case SpamTrawler_Cache::CLEANING_MODE_ALL:
                $boolFast = $this->_fastBackend->clean(SpamTrawler_Cache::CLEANING_MODE_ALL);
                $boolSlow = $this->_slowBackend->clean(SpamTrawler_Cache::CLEANING_MODE_ALL);
                return $boolFast && $boolSlow;
                break;
            case SpamTrawler_Cache::CLEANING_MODE_OLD:
                return $this->_slowBackend->clean(SpamTrawler_Cache::CLEANING_MODE_OLD);
