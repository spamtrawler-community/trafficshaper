 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Cache_Frontend_Function extends SpamTrawler_Cache_Core
{
    /**
    ...
    public function call($callback, array $parameters = array(), $tags = array(), $specificLifetime = false, $priority = 8)
    {
        if (!is_callable($callback, true, $name)) {
            SpamTrawler_Cache::throwException('Invalid callback');
        }
    ...
    public function makeId($callback, array $args = array())
    {
        ...
        if (isset($object)) {
            try {
                ...
                SpamTrawler_Cache::throwException($e->getMessage());
            }
            if (!$tmp) {
                $lastErr = error_get_last();
                SpamTrawler_Cache::throwException("Can't serialize callback object to generate id: {$lastErr['message']}");
            }
            $name.= '__' . $tmp;
        }
        ...
        if ($args) {
            ...
            if (!$argsStr) {
                $lastErr = error_get_last();
                throw SpamTrawler_Cache::throwException("Can't serialize arguments to generate id: {$lastErr['message']}");
            }
        }
