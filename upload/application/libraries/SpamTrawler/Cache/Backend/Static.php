 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Cache_Backend_Static
    extends SpamTrawler_Cache_Backend
    implements SpamTrawler_Cache_Backend_Interface
{
    const INNER_CACHE_NAME = 'SpamTrawler_Cache_backend_static_tagcache';
     ...
     * @param  string $name
     * @param  mixed $value
     * @return SpamTrawler_Cache_Backend_Static
     */
    ...
    public function load($id, $doNotTestCacheValidity = false)
    {
        ...
        if (!$this->_verifyPath($id)) {
            SpamTrawler_Cache::throwException('Invalid cache id: does not match expected public_dir path');
        }
        if ($doNotTestCacheValidity) {
            $this->_log("SpamTrawler_Cache_Backend_Static::load() : \$doNotTestCacheValidity=true is unsupported by the Static backend");
        }
    ...
    protected function _createDirectoriesFor($path)
    {
        if (!is_dir($path)) {
            ...
            if ( !@mkdir($path, $this->_octdec($this->_options['cache_directory_perm']), true)) {
                ...
                umask($oldUmask);
                SpamTrawler_Cache::throwException("Can't create directory: {$lastErr['message']}");
            }
            umask($oldUmask);
        }
    }
    ...
    public function clean($mode = SpamTrawler_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        ...
        switch ($mode) {
            ...
            case SpamTrawler_Cache::CLEANING_MODE_MATCHING_ANY_TAG:
                if (empty($tags)) {
                    throw new SpamTrawler_Exception('Cannot use tag matching modes as no tags were defined');
                }
                if ($this->_tagged === null && $tagged = $this->getInnerCache()->load(self::INNER_CACHE_NAME)) {
                    $this->_tagged = $tagged;
                } elseif (!$this->_tagged) {
            ...
            case SpamTrawler_Cache::CLEANING_MODE_OLD:
                $this->_log("SpamTrawler_Cache_Backend_Static : Selected Cleaning Mode Currently Unsupported By This Backend");
                break;
     ...
     *
     * @param  SpamTrawler_Cache_Core
     * @return void
     */
    public function setInnerCache(SpamTrawler_Cache_Core $cache)
    {
    ...
    public function getInnerCache()
    {
        if ($this->_tagCache === null) {
            SpamTrawler_Cache::throwException('An Inner Cache has not been set; use setInnerCache()');
        }
    ...
    protected static function _validateIdOrTag($string)
    {
        ...
        if (!preg_match(
            ...
            SpamTrawler_Cache::throwException("Invalid id or tag '$string' : must be a valid URL path");
        }
    }
