 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Filter_Encrypt_Openssl implements SpamTrawler_Filter_Encrypt_Interface
{
    /**
    ...
    public function __construct($options = array())
    {
        if (!extension_loaded('openssl')) {
            ...
            throw new SpamTrawler_Filter_Exception('This filter needs the openssl extension');
        }
     ...
     * @return SpamTrawler_Filter_Encrypt_Openssl
     */
        ...
        foreach ($keys as $type => $key) {
            ...
            switch ($type) {
                case 'public':
                    ...
                    if ($test === false) {
                        require_once 'SpamTrawler/Filter/Exception.php';
                        throw new SpamTrawler_Filter_Exception("Public key '{$cert}' not valid");
                    }
                ...
                case 'private':
                    ...
                    if ($test === false) {
                        require_once 'SpamTrawler/Filter/Exception.php';
                        throw new SpamTrawler_Filter_Exception("Private key '{$cert}' not valid");
                    }
    ...
    public function encrypt($value)
    {
        ...
        if (count($this->_keys['public']) == 0) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('Openssl can not encrypt without public keys');
        }
        ...
        foreach($this->_keys['public'] as $key => $cert) {
            ...
            if ($this->_package) {
                ...
                if ($details === false) {
                    $details = array('key' => 'SpamTrawlerFramework');
                }
        ...
        if ($crypt === false) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('Openssl was not able to encrypt your content with the given options');
        }
    ...
    public function decrypt($value)
    {
        ...
        if (count($this->_keys['private']) !== 1) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('Please give a private key for decryption with Openssl');
        }

        if (!$this->_package && empty($envelope)) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('Please give a envelope key for decryption with Openssl');
        }
        ...
        if ($this->_package) {
            ...
            if ($details !== false) {
                ...
                $fingerprint = md5("SpamTrawlerFramework");
            }
        ...
        if ($crypt === false) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('Openssl was not able to decrypt you content with the given options');
        }
