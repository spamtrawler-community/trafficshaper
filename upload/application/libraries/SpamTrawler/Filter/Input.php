...
/**
 * @see SpamTrawler_Filter
 */
require_once 'SpamTrawler/Filter.php';

/**
 * @see SpamTrawler_Validate
 */
require_once 'SpamTrawler/Validate.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Filter_Input
{
    ...
    /**
     * @var SpamTrawler_Filter_Interface The filter object that is run on values
     ...
     * @return SpamTrawler_Filter_Input
     ...
     *
     * @param  SpamTrawler_Loader_PluginLoader_Interface $loader
     * @param  string $type 'filter' or 'validate'
     * @return SpamTrawler_Filter_Input
     * @throws SpamTrawler_Filter_Exception on invalid type
     */
    public function setPluginLoader(SpamTrawler_Loader_PluginLoader_Interface $loader, $type)
    {
        ...
        switch ($type) {
            ...
            default:
                ...
                throw new SpamTrawler_Filter_Exception(sprintf('Invalid type "%s" provided to setPluginLoader()', $type));
        }
     ...
     *
     * @param  string $type 'filter' or 'validate'
     * @return SpamTrawler_Loader_PluginLoader_Interface
     * @throws SpamTrawler_Filter_Exception on invalid type
     */
    public function getPluginLoader($type)
    {
        ...
        if (!isset($this->_loaders[$type])) {
            switch ($type) {
                case self::FILTER:
                    $prefixSegment = 'SpamTrawler_Filter_';
                    $pathSegment   = 'SpamTrawler/Filter/';
                    break;
                case self::VALIDATE:
                    $prefixSegment = 'SpamTrawler_Validate_';
                    $pathSegment   = 'SpamTrawler/Validate/';
                    break;
                default:
                    require_once 'SpamTrawler/Filter/Exception.php';
                    throw new SpamTrawler_Filter_Exception(sprintf('Invalid type "%s" provided to getPluginLoader()', $type));
            }

            require_once 'SpamTrawler/Loader/PluginLoader.php';
            $this->_loaders[$type] = new SpamTrawler_Loader_PluginLoader(
    ...
    /**
     * @return SpamTrawler_Filter_Input
     * @throws SpamTrawler_Filter_Exception
     */
    public function process()
    {
        ...
        if ($this->hasInvalid()) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception("Input has invalid fields");
        }
        if ($this->hasMissing()) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception("Input has missing fields");
        }
     ...
     * @return SpamTrawler_Filter_Interface
     */
    public function setDefaultEscapeFilter($escapeFilter)
    {
        ...
        if (!$escapeFilter instanceof SpamTrawler_Filter_Interface) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception('Escape filter specified does not implement SpamTrawler_Filter_Interface');
        }
     ...
     * @param array $options
     * @return SpamTrawler_Filter_Input
     * @throws SpamTrawler_Filter_Exception if an unknown option is given
     */
    public function setOptions(array $options)
    {
        foreach ($options as $option => $value) {
            switch ($option) {
                ...
                default:
                    require_once 'SpamTrawler/Filter/Exception.php';
                    throw new SpamTrawler_Filter_Exception("Unknown option '$option'");
                    break;
            }
        }
     ...
     *
     * @param  SpamTrawler_Translate|SpamTrawler_Translate_Adapter|null $translator
     * @return SpamTrawler_Filter_Input
     */
    public function setTranslator($translator = null)
    {
        if ((null === $translator) || ($translator instanceof SpamTrawler_Translate_Adapter)) {
            $this->_translator = $translator;
        } elseif ($translator instanceof SpamTrawler_Translate) {
            $this->_translator = $translator->getAdapter();
        } else {
            require_once 'SpamTrawler/Validate/Exception.php';
            throw new SpamTrawler_Validate_Exception('Invalid translator specified');
        }
     ...
     *
     * @return SpamTrawler_Translate_Adapter|null
     */
    public function getTranslator()
    {
        ...
        if ($this->_translator === null) {
            ...
            if (SpamTrawler_Registry::isRegistered('SpamTrawler_Translate')) {
                $translator = SpamTrawler_Registry::get('SpamTrawler_Translate');
                if ($translator instanceof SpamTrawler_Translate_Adapter) {
                    return $translator;
                } elseif ($translator instanceof SpamTrawler_Translate) {
    ...
    protected function _filter()
    {
        foreach ($this->_filterRules as $ruleName => &$filterRule) {
             ...
             * Don't typecast to (array) because it might be a SpamTrawler_Filter object
             */
            ...
            if (!isset($filterRule[self::FILTER_CHAIN])) {
                $filterRule[self::FILTER_CHAIN] = new SpamTrawler_Filter();
    ...
    protected function _validate()
    {
        ...
        foreach ($this->_validatorRules as $ruleName => &$validatorRule) {
             ...
             * Don't typecast to (array) because it might be a SpamTrawler_Validate object
             */
            ...
            if (!isset($validatorRule[self::ALLOW_EMPTY])) {
                ...
                foreach ($validatorRule as $rule) {
                    ...
                    if($rule instanceof SpamTrawler_Validate_NotEmpty) {
                        $foundNotEmptyValidator = true;
                        // field may not be empty, we are ready
                        break 1;
                    }
            ...
            if (!isset($validatorRule[self::VALIDATOR_CHAIN])) {
                $validatorRule[self::VALIDATOR_CHAIN] = new SpamTrawler_Validate();
                ...
                foreach ($validatorList as $key => $validator) {
                    ...
                    if (isset($validatorRule[self::MESSAGES][$key])) {
                        ...
                        if ($validator instanceof SpamTrawler_Validate_NotEmpty) {
        ...
        if (count((array) $validatorRule[self::FIELDS]) > 1) {
            if (!$validatorRule[self::ALLOW_EMPTY]) {
                ...
                foreach ($data as $fieldKey => $field) {
                    // if there is no SpamTrawler_Validate_NotEmpty instance in the rules, we will use the default
            ...
            if ($validatorRule[self::ALLOW_EMPTY]) {
                ...
                $validatorChain = new SpamTrawler_Validate();
     ...
     * set on the validator outside SpamTrawler_Filter_Input.
     * @param array $validatorRule
     * @return mixed false if none is found, SpamTrawler_Validate_NotEmpty instance if found
     */
    protected function _getNotEmptyValidatorInstance($validatorRule) {
        foreach ($validatorRule as $rule => $value) {
            if (is_object($value) and $value instanceof SpamTrawler_Validate_NotEmpty) {
                return $value;
            }
        }
    ...
    /**
     * @param mixed $classBaseName
     * @return SpamTrawler_Validate_Interface
     */
     ...
     * @param string $type
     * @param mixed $classBaseName
     * @return SpamTrawler_Filter_Interface|SpamTrawler_Validate_Interface
     * @throws SpamTrawler_Filter_Exception
     */
    protected function _getFilterOrValidator($type, $classBaseName)
    {
        ...
        $interfaceName = 'SpamTrawler_' . ucfirst($type) . '_Interface';
        ...
        if (!$class->implementsInterface($interfaceName)) {
            require_once 'SpamTrawler/Filter/Exception.php';
            throw new SpamTrawler_Filter_Exception("Class '$className' based on basename '$classBaseName' must implement the '$interfaceName' interface");
        }
