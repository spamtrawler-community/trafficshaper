...
/**
 * @see SpamTrawler_Db_Adapter_Abstract
 */
require_once 'SpamTrawler/Db/Adapter/Abstract.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class SpamTrawler_Db_Table_Abstract
{
    ...
    /**
     * Default SpamTrawler_Db_Adapter_Abstract object.
     *
     * @var SpamTrawler_Db_Adapter_Abstract
     */
    protected static $_defaultDb;

    /**
     * Optional SpamTrawler_Db_Table_Definition object
     *
     * @var unknown_type
     */
     ...
     *
     * @var SpamTrawler_Cache_Core
     */
    protected static $_defaultMetadataCache = null;

    /**
     * SpamTrawler_Db_Adapter_Abstract object.
     *
     * @var SpamTrawler_Db_Adapter_Abstract
     */
    protected $_db;
    ...
    /**
     * The table column names derived from SpamTrawler_Db_Adapter_Abstract::describeTable().
     *
     * @var array
     */
    protected $_cols;
     ...
     * @var string
     */
    protected $_rowClass = 'SpamTrawler_Db_Table_Row';
     ...
     * @var string
     */
    protected $_rowsetClass = 'SpamTrawler_Db_Table_Rowset';
     ...
     * extend SpamTrawler_Db_Table_Abstract.
     *
     * @var array
     */
     ...
     * setDefinition()
     *
     * @param SpamTrawler_Db_Table_Definition $definition
     * @return SpamTrawler_Db_Table_Abstract
     */
    public function setDefinition(SpamTrawler_Db_Table_Definition $definition)
    {
     ...
     * getDefinition()
     *
     * @return SpamTrawler_Db_Table_Definition|null
     */
     ...
     * @return SpamTrawler_Db_Table_Abstract Provides a fluent interface
     */
     ...
     * @return array
     * @throws SpamTrawler_Db_Table_Exception
     */
    public function getReference($tableClassname, $ruleKey = null)
    {
        $thisClass = get_class($this);
        if ($thisClass === 'SpamTrawler_Db_Table') {
            $thisClass = $this->_definitionConfigName;
        }
        $refMap = $this->_getReferenceMapNormalized();
        if ($ruleKey !== null) {
            if (!isset($refMap[$ruleKey])) {
                require_once "SpamTrawler/Db/Table/Exception.php";
                throw new SpamTrawler_Db_Table_Exception("No reference rule \"$ruleKey\" from table $thisClass to table $tableClassname");
            }
            if ($refMap[$ruleKey][self::REF_TABLE_CLASS] != $tableClassname) {
                require_once "SpamTrawler/Db/Table/Exception.php";
                throw new SpamTrawler_Db_Table_Exception("Reference rule \"$ruleKey\" does not reference table $tableClassname");
            }
        ...
        require_once "SpamTrawler/Db/Table/Exception.php";
        throw new SpamTrawler_Db_Table_Exception("No reference from table $thisClass to table $tableClassname");
    }
    ...
    /**
     * Sets the default SpamTrawler_Db_Adapter_Abstract for all SpamTrawler_Db_Table objects.
     *
    ...
    /**
     * Gets the default SpamTrawler_Db_Adapter_Abstract for all SpamTrawler_Db_Table objects.
     *
     * @return SpamTrawler_Db_Adapter_Abstract or null
     */
    ...
    /**
     * Gets the SpamTrawler_Db_Adapter_Abstract for this particular SpamTrawler_Db_Table object.
     *
     * @return SpamTrawler_Db_Adapter_Abstract
     */
    ...
    protected static function _setupAdapter($db)
    {
        ...
        if (is_string($db)) {
            ...
            $db = SpamTrawler_Registry::get($db);
        }
        if (!$db instanceof SpamTrawler_Db_Adapter_Abstract) {
            require_once 'SpamTrawler/Db/Table/Exception.php';
            throw new SpamTrawler_Db_Table_Exception('Argument must be of type SpamTrawler_Db_Adapter_Abstract, or a Registry key where a SpamTrawler_Db_Adapter_Abstract object is stored');
        }
        return $db;
    }

    /**
     * Sets the default metadata cache for information returned by SpamTrawler_Db_Adapter_Abstract::describeTable().
     *
    ...
    /**
     * Gets the default metadata cache for information returned by SpamTrawler_Db_Adapter_Abstract::describeTable().
     *
     * @return SpamTrawler_Cache_Core or null
     */
    ...
    /**
     * Sets the metadata cache for information returned by SpamTrawler_Db_Adapter_Abstract::describeTable().
     *
    ...
    /**
     * Gets the metadata cache for information returned by SpamTrawler_Db_Adapter_Abstract::describeTable().
     *
     * @return SpamTrawler_Cache_Core or null
     */
    ...
    protected static function _setupMetadataCache($metadataCache)
    {
        ...
        if (is_string($metadataCache)) {
            require_once 'SpamTrawler/Registry.php';
            $metadataCache = SpamTrawler_Registry::get($metadataCache);
        }
        if (!$metadataCache instanceof SpamTrawler_Cache_Core) {
            require_once 'SpamTrawler/Db/Table/Exception.php';
            throw new SpamTrawler_Db_Table_Exception('Argument must be of type SpamTrawler_Cache_Core, or a Registry key where a SpamTrawler_Cache_Core object is stored');
        }
     ...
     * @return SpamTrawler_Db_Table_Adapter_Abstract Provides a fluent interface
     */
    ...
    protected function _setupDatabaseAdapter()
    {
        if (! $this->_db) {
            ...
            if (!$this->_db instanceof SpamTrawler_Db_Adapter_Abstract) {
                require_once 'SpamTrawler/Db/Table/Exception.php';
                throw new SpamTrawler_Db_Table_Exception('No adapter found for ' . get_class($this));
            }
        }
    }
    ...
    protected function _setupPrimaryKey()
    {
        if (!$this->_primary) {
            ...
            if (empty($this->_primary)) {
                require_once 'SpamTrawler/Db/Table/Exception.php';
                throw new SpamTrawler_Db_Table_Exception("A table must have a primary key, but none was found for table '{$this->_name}'");
            }
        ...
        if (! array_intersect((array) $this->_primary, $cols) == (array) $this->_primary) {
            require_once 'SpamTrawler/Db/Table/Exception.php';
            throw new SpamTrawler_Db_Table_Exception("Primary key column(s) ("
         ...
         */
        if ($this->_sequence === true && $this->_db instanceof SpamTrawler_Db_Adapter_Pdo_Pgsql) {
    ...
    public function info($key = null)
    {
        ...
        if (!array_key_exists($key, $info)) {
            require_once 'SpamTrawler/Db/Table/Exception.php';
            throw new SpamTrawler_Db_Table_Exception('There is no table information for the key "' . $key . '"');
        }
    ...
    /**
     * Returns an instance of a SpamTrawler_Db_Table_Select object.
     *
    ...
    {
        require_once 'SpamTrawler/Db/Table/Select.php';
        $select = new SpamTrawler_Db_Table_Select($this);
        if ($withFromPart == self::SELECT_WITH_FROM_PART) {
            $select->from($this->info(self::NAME), SpamTrawler_Db_Table_Select::SQL_WILDCARD, $this->info(self::SCHEMA));
        }
        return $select;
    }
    ...
    public function insert(array $data)
    {
        ...
        /**
         * SpamTrawler_Db_Table assumes that if you have a compound primary key
    ...
    public function isIdentity($column)
    {
        ...
        if (!isset($this->_metadata[$column])) {
            /**
             * @see SpamTrawler_Db_Table_Exception
             */
            require_once 'SpamTrawler/Db/Table/Exception.php';

            throw new SpamTrawler_Db_Table_Exception('Column "' . $column . '" not found in table.');
        }
     ...
     * @return SpamTrawler_Db_Table_Rowset_Abstract Row(s) matching the criteria.
     * @throws SpamTrawler_Db_Table_Exception
     */
    public function find()
    {
        ...
        if (count($args) < count($keyNames)) {
            require_once 'SpamTrawler/Db/Table/Exception.php';
            throw new SpamTrawler_Db_Table_Exception("Too few columns for the primary key");
        }

        if (count($args) > count($keyNames)) {
            require_once 'SpamTrawler/Db/Table/Exception.php';
            throw new SpamTrawler_Db_Table_Exception("Too many columns for the primary key");
        }
        ...
        foreach ($args as $keyPosition => $keyValues) {
            ...
            // might be SpamTrawler_Db_Expr objects.
            ...
            if ($numberTerms == 0) {
                ...
                require_once 'SpamTrawler/Db/Table/Exception.php';
                throw new SpamTrawler_Db_Table_Exception("Missing value(s) for the primary key");
            }
     ...
     * Fetches all rows.
     *
     * Honors the SpamTrawler_Db_Adapter fetch mode.
     *
     * @param string|array|SpamTrawler_Db_Table_Select $where  OPTIONAL An SQL WHERE clause or SpamTrawler_Db_Table_Select object.
     * @param string|array                      $order  OPTIONAL An SQL ORDER clause.
     * @param int                               $count  OPTIONAL An SQL LIMIT count.
     * @param int                               $offset OPTIONAL An SQL LIMIT offset.
     * @return SpamTrawler_Db_Table_Rowset_Abstract The row results per the SpamTrawler_Db_Adapter fetch mode.
     */
    public function fetchAll($where = null, $order = null, $count = null, $offset = null)
    {
        if (!($where instanceof SpamTrawler_Db_Table_Select)) {
            $select = $this->select();
    ...
    /**
     * Fetches one row in an object of type SpamTrawler_Db_Table_Row_Abstract,
     * or returns null if no row matches the specified criteria.
     *
     * @param string|array|SpamTrawler_Db_Table_Select $where  OPTIONAL An SQL WHERE clause or SpamTrawler_Db_Table_Select object.
     * @param string|array                      $order  OPTIONAL An SQL ORDER clause.
     * @param int                               $offset OPTIONAL An SQL OFFSET value.
     * @return SpamTrawler_Db_Table_Row_Abstract|null The row results per the
     *     SpamTrawler_Db_Adapter fetch mode, or null if no row found.
     */
    public function fetchRow($where = null, $order = null, $offset = null)
    {
        if (!($where instanceof SpamTrawler_Db_Table_Select)) {
            ...
            $select = $where->limit(1, $where->getPart(SpamTrawler_Db_Select::LIMIT_OFFSET));
        }
        ...
        if (!class_exists($rowClass)) {
            require_once 'SpamTrawler/Loader.php';
            SpamTrawler_Loader::loadClass($rowClass);
        }
     ...
     * @return SpamTrawler_Db_Table_Select
     */
    protected function _where(SpamTrawler_Db_Table_Select $select, $where)
    {
        $where = (array) $where;
     ...
     * @return SpamTrawler_Db_Table_Select
     */
    protected function _order(SpamTrawler_Db_Table_Select $select, $order)
    {
        if (!is_array($order)) {
            $order = array($order);
        }
     ...
     *
     * @param  SpamTrawler_Db_Table_Select $select  query options.
     * @return array An array containing the row results in FETCH_ASSOC mode.
     */
    protected function _fetch(SpamTrawler_Db_Table_Select $select)
    {
    ...
    public static function getTableFromString($tableName, SpamTrawler_Db_Table_Abstract $referenceTable = null)
    {
        if ($referenceTable instanceof SpamTrawler_Db_Table_Abstract) {
            $tableDefinition = $referenceTable->getDefinition();

            if ($tableDefinition !== null && $tableDefinition->hasTableConfig($tableName)) {
                return new SpamTrawler_Db_Table($tableName, $tableDefinition);
            }
        }
        ...
        if (!class_exists($tableName)) {
            try {
                require_once 'SpamTrawler/Loader.php';
                SpamTrawler_Loader::loadClass($tableName);
        ...
        if (isset($tableDefinition) && $tableDefinition !== null) {
            $options[SpamTrawler_Db_Table_Abstract::DEFINITION] = $tableDefinition;
        }
