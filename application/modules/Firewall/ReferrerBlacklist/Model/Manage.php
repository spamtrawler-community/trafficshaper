...
class Firewall_ReferrerBlacklist_Model_Manage extends Firewall_Abstract_Model_Manage
{
    protected $dbTableName = 'blacklist_referrer';
    protected $dbTableNameValueCache = 'cache_referrer';
    protected $dbFieldNameValueVisitor = 'referrerID';
    protected $dbFieldNameValueCache = 'referrer';
    public $oInput = NULL;
    protected $aOutput = NULL;
    public $sOutput = NULL;
    public $sErrors = NULL;
    protected $sDbFilterField = 'referrer';
    ...
    {
        $this->aOutput = array(
            'id' => md5($this->oInput[0]->referrer),
            'referrer' => $this->oInput[0]->referrer,
    ...
    public function validate()
    {
        ...
        if(!in_array($this->oInput[0]->filter_mode, $aAllowedFilterModes)){
            ...
            try{
                ...
                new SpamTrawler_Validate_Regex(array('pattern' => $this->oInput[0]->referrer));
            } catch(Exception $e) {
                $aErrors = array(
            ...
            if(!filter_var($this->oInput[0]->referrer, FILTER_VALIDATE_URL)){
                $aErrors = array(
    ...
    public static function validateSettings($aSettings){
        ...
        if(!ctype_digit($aSettings['conf_status'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Referrer Blacklist Status Invalid!';
        }

        if(!ctype_digit($aSettings['conf_params']['allowcaptcha'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Referrer Blacklist Allow Captcha Invalid!';
        }

        $sBlockReason = str_replace(' ', '', $aSettings['conf_params']['block_reason']);
        if(!ctype_alnum($sBlockReason)){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Referrer Blacklist Block Reason Invalid!';
        }

        /*
        if(!ctype_digit($aSettings['conf_order'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Referrer Blacklist Order Invalid!';
        }
        */
