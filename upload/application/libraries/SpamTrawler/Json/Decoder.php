 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Json_Decoder
{
    /**
     ...
     * {@link SpamTrawler_Json::TYPE_ARRAY} and {@link SpamTrawler_Json::TYPE_OBJECT} for
     * valid values
     * @return void
     */
    protected function __construct($source, $decodeType)
    {
        ...
        if (!in_array($decodeType, array(SpamTrawler_Json::TYPE_ARRAY, SpamTrawler_Json::TYPE_OBJECT)))
        {
            $decodeType = SpamTrawler_Json::TYPE_ARRAY;
        }
     ...
     * return a StdClass object instead, pass {@link SpamTrawler_Json::TYPE_OBJECT} to
     * the $objectDecodeType parameter.
     *
     * Throws a SpamTrawler_Json_Exception if the source string is null.
     *
     * @static
     * @access public
     * @param string $source String to be decoded
     * @param int $objectDecodeType How objects should be decoded; should be
     * either or {@link SpamTrawler_Json::TYPE_ARRAY} or
     * {@link SpamTrawler_Json::TYPE_OBJECT}; defaults to TYPE_ARRAY
     * @return mixed
     * @throws SpamTrawler_Json_Exception
     */
    public static function decode($source = null, $objectDecodeType = SpamTrawler_Json::TYPE_ARRAY)
    {
        if (null === $source) {
            require_once 'SpamTrawler/Json/Exception.php';
            throw new SpamTrawler_Json_Exception('Must specify JSON encoded source for decoding');
        } elseif (!is_string($source)) {
            require_once 'SpamTrawler/Json/Exception.php';
            throw new SpamTrawler_Json_Exception('Can only decode JSON encoded strings');
        }
     ...
     *
     * If SpamTrawler_Json_Encoder was used to encode the original object then
    ...
    protected function _decodeObject()
    {
        ...
        while ($tok && $tok != self::RBRACE) {
            if ($tok != self::DATUM || ! is_string($this->_tokenValue)) {
                require_once 'SpamTrawler/Json/Exception.php';
                throw new SpamTrawler_Json_Exception('Missing key in object encoding: ' . $this->_source);
            }
            ...
            if ($tok != self::COLON) {
                require_once 'SpamTrawler/Json/Exception.php';
                throw new SpamTrawler_Json_Exception('Missing ":" in object encoding: ' . $this->_source);
            }
        ...
        switch ($this->_decodeType) {
            case SpamTrawler_Json::TYPE_OBJECT:
                ...
                break;
            case SpamTrawler_Json::TYPE_ARRAY:
            default:
                $result = $members;
                break;
        }
    ...
    protected function _decodeArray()
    {
        ...
        while ($tok && $tok != self::RBRACKET) {
            ...
            if ($tok != self::COMMA) {
                require_once 'SpamTrawler/Json/Exception.php';
                throw new SpamTrawler_Json_Exception('Missing "," in array encoding: ' . $this->_source);
            }
    ...
    protected function _getNextToken()
    {
                ...
                do {
                    ...
                    if ($chr == '\\') {
                        ...
                        switch ($chr) {
                            ...
                            default:
                                require_once 'SpamTrawler/Json/Exception.php';
                                throw new SpamTrawler_Json_Exception("Illegal escape "
                                    .  "sequence '" . $chr . "'");
                        }
                    } elseif($chr == '"') {
                        break;
        ...
        if ($chr == '-' || $chr == '.' || ($chr >= '0' && $chr <= '9')) {
            if (preg_match('/-?([0-9])*(\.[0-9]*)?((e|E)((-|\+)?)[0-9]+)?/s',
                ...
                if (is_numeric($datum)) {
                    if (preg_match('/^0\d+$/', $datum)) {
                        require_once 'SpamTrawler/Json/Exception.php';
                        throw new SpamTrawler_Json_Exception("Octal notation not supported by JSON (value: $datum)");
                    } else {
                        $val  = intval($datum);
                        $fVal = floatval($datum);
                    ...
                    require_once 'SpamTrawler/Json/Exception.php';
                    throw new SpamTrawler_Json_Exception("Illegal number format: $datum");
                }
            ...
            require_once 'SpamTrawler/Json/Exception.php';
            throw new SpamTrawler_Json_Exception('Illegal Token');
        }
