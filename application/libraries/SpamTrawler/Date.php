 ...
 */
require_once 'SpamTrawler/Date/DateObject.php';
require_once 'SpamTrawler/Locale.php';
require_once 'SpamTrawler/Locale/Format.php';
require_once 'SpamTrawler/Locale/Math.php';
 ...
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class SpamTrawler_Date extends SpamTrawler_Date_DateObject
{
     ...
     *
     * @param  string|integer|SpamTrawler_Date|array  $date    OPTIONAL Date value or value of date part to set
     *                                                 ,depending on $part. If null the actual time is set
     * @param  string                          $part    OPTIONAL Defines the input format of $date
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date
     * @throws SpamTrawler_Date_Exception
     */
    public function __construct($date = null, $part = null, $locale = null)
    {
        if (is_object($date) and !($date instanceof SpamTrawler_TimeSync_Protocol) and
            !($date instanceof SpamTrawler_Date)) {
            if ($locale instanceof SpamTrawler_Locale) {
                $locale = $date;
                $date   = null;
                $part   = null;
            } else {
        ...
        if (($date !== null) and !is_array($date) and !($date instanceof SpamTrawler_TimeSync_Protocol) and
            !($date instanceof SpamTrawler_Date) and !defined($date) and SpamTrawler_Locale::isLocale($date, true, false)) {
            $locale = $date;
            $date   = null;
            $part   = null;
        } else if (($part !== null) and !defined($part) and SpamTrawler_Locale::isLocale($part, true, false)) {
            $locale = $part;
            $part   = null;
        }
        ...
        if ($date instanceof SpamTrawler_TimeSync_Protocol) {
            $date = $date->getInfo();
    ...
    public static function setOptions(array $options = array())
    {
        ...
        foreach ($options as $name => $value) {
            ...
            if (array_key_exists($name, self::$_options)) {
                switch($name) {
                    case 'format_type' :
                        if ((strtolower($value) != 'php') && (strtolower($value) != 'iso')) {
                            require_once 'SpamTrawler/Date/Exception.php';
                            throw new SpamTrawler_Date_Exception("Unknown format type ($value) for dates, only 'iso' and 'php' supported", 0, null, $value);
                        }
                        break;
                    case 'fix_dst' :
                        if (!is_bool($value)) {
                            require_once 'SpamTrawler/Date/Exception.php';
                            throw new SpamTrawler_Date_Exception("'fix_dst' has to be boolean", 0, null, $value);
                        }
                        break;
                    case 'extend_month' :
                        if (!is_bool($value)) {
                            require_once 'SpamTrawler/Date/Exception.php';
                            throw new SpamTrawler_Date_Exception("'extend_month' has to be boolean", 0, null, $value);
                        }
                        break;
                    case 'cache' :
                        if ($value === null) {
                            ...
                            if (!$value instanceof SpamTrawler_Cache_Core) {
                                require_once 'SpamTrawler/Date/Exception.php';
                                throw new SpamTrawler_Date_Exception("Instance of SpamTrawler_Cache expected");
                            }

                            parent::$_cache = $value;
                            parent::$_cacheTags = SpamTrawler_Date_DateObject::_getTagSupportForCache();
                            SpamTrawler_Locale_Data::setCache($value);
                        }
                        break;
                    case 'timesync' :
                        if ($value === null) {
                            ...
                            if (!$value instanceof SpamTrawler_TimeSync_Protocol) {
                                require_once 'SpamTrawler/Date/Exception.php';
                                throw new SpamTrawler_Date_Exception("Instance of SpamTrawler_TimeSync expected");
                            }
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("Unknown option: $name = $value");
            }
        }
    }

    /**
     * Returns this object's internal UNIX timestamp (equivalent to SpamTrawler_Date::TIMESTAMP).
     ...
     * @param  string|integer|array|SpamTrawler_Date  $stamp   Timestamp to calculate, when null the actual timestamp is calculated
     * @return SpamTrawler_Date|integer
     * @throws SpamTrawler_Date_Exception
     */
    private function _timestamp($calc, $stamp)
    {
        if ($stamp instanceof SpamTrawler_Date) {
        ...
        if (is_array($stamp)) {
            if (isset($stamp['timestamp']) === true) {
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception('no timestamp given in array');
            }
        }
     ...
     *
     * @param  integer|string|array|SpamTrawler_Date  $timestamp  Timestamp to set
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Adds a timestamp
     *
     * @param  integer|string|array|SpamTrawler_Date  $timestamp  Timestamp to add
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  integer|string|array|SpamTrawler_Date  $timestamp  Timestamp to sub
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  integer|string|array|SpamTrawler_Date  $timestamp  Timestamp to compare
    ...
    {
        if (is_object($format)) {
            if ($format instanceof SpamTrawler_Locale) {
                $locale = $format;
                $format = null;
            } else {
        ...
        if (is_object($type)) {
            if ($type instanceof SpamTrawler_Locale) {
                $locale = $type;
                $type   = null;
            } else {
                $type = (string) $type;
        ...
        if (($format !== null) && !defined($format)
            ...
            && SpamTrawler_Locale::isLocale($format, null, false)) {
            $locale = $format;
            $format = null;
        }

        if (($type !== null) and ($type != 'php') and ($type != 'iso') and
            SpamTrawler_Locale::isLocale($type, null, false)) {
            $locale = $type;
            $type = null;
        }
        ...
        if ($format === null) {
            $format = SpamTrawler_Locale_Format::getDateFormat($locale) . ' ' . SpamTrawler_Locale_Format::getTimeFormat($locale);
        } else if (((self::$_options['format_type'] == 'php') && ($type === null)) or ($type == 'php')) {
            $format = SpamTrawler_Locale_Format::convertPhpToIsoFormat($format);
        }
     ...
     *
     * @param  string|integer|SpamTrawler_Date  $part  OPTIONAL Defines the date or datepart to return as integer
    ...
    public function get($part = null, $locale = null)
    {
        ...
        if (($part !== null) && !defined($part)
            ...
            && SpamTrawler_Locale::isLocale($part, null, false)) {
            $locale = $part;
            $part = null;
        }
        ...
        if ($part === null) {
            ...
            $part = SpamTrawler_Locale_Format::convertPhpToIsoFormat($part);
        }
    ...
    private function _parseIsoToDate($token, $locale) {
        switch($token) {
            ...
            case self::WEEKDAY_SHORT :
                ...
                $day     = SpamTrawler_Locale_Data::getContent($locale, 'day', array('gregorian', 'format', 'wide', $weekday));
            ...
            case self::WEEKDAY :
                $weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
                return $this->_toComment(SpamTrawler_Locale_Data::getContent($locale, 'day', array('gregorian', 'format', 'wide', $weekday)));
                break;
            ...
            case 'EEEEE' :
                $weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
                $day = SpamTrawler_Locale_Data::getContent($locale, 'day', array('gregorian', 'format', 'abbreviated', $weekday));
                return $this->_toComment(iconv_substr($day, 0, 1, 'UTF-8'));
                break;

            case self::WEEKDAY_NAME :
                $weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
                return $this->_toComment(SpamTrawler_Locale_Data::getContent($locale, 'day', array('gregorian', 'format', 'abbreviated', $weekday)));
                break;
            ...
            case self::MONTH_NAME :
                ...
                return $this->_toComment(SpamTrawler_Locale_Data::getContent($locale, 'month', array('gregorian', 'format', 'wide', $month)));
                break;
            ...
            case self::MONTH_NAME_SHORT :
                $month = $this->date('n', $this->getUnixTimestamp(), false);
                return $this->_toComment(SpamTrawler_Locale_Data::getContent($locale, 'month', array('gregorian', 'format', 'abbreviated', $month)));
                break;
            ...
            case self::MONTH_NAME_NARROW :
                $month = $this->date('n', $this->getUnixTimestamp(), false);
                $mon = SpamTrawler_Locale_Data::getContent($locale, 'month', array('gregorian', 'format', 'abbreviated', $month));
            ...
            case self::MERIDIEM :
                ...
                if ($am == 'am') {
                    return $this->_toComment(SpamTrawler_Locale_Data::getContent($locale, 'am'));
                }

                return $this->_toComment(SpamTrawler_Locale_Data::getContent($locale, 'pm'));
                break;
            ...
            case 'GGG' :
                ...
                if ($year < 0) {
                    return $this->_toComment(SpamTrawler_Locale_Data::getContent($locale, 'era', array('gregorian', 'Abbr', '0')));
                }

                return $this->_toComment(SpamTrawler_Locale_Data::getContent($locale, 'era', array('gregorian', 'Abbr', '1')));
                break;

            case self::ERA_NARROW :
                $year = $this->date('Y', $this->getUnixTimestamp(), false);
                if ($year < 0) {
                    return $this->_toComment(iconv_substr(SpamTrawler_Locale_Data::getContent($locale, 'era', array('gregorian', 'Abbr', '0')), 0, 1, 'UTF-8')) . '.';
                }

                return $this->_toComment(iconv_substr(SpamTrawler_Locale_Data::getContent($locale, 'era', array('gregorian', 'Abbr', '1')), 0, 1, 'UTF-8')) . '.';
                break;

            case self::ERA_NAME :
                $year = $this->date('Y', $this->getUnixTimestamp(), false);
                if ($year < 0) {
                    return $this->_toComment(SpamTrawler_Locale_Data::getContent($locale, 'era', array('gregorian', 'Names', '0')));
                }

                return $this->_toComment(SpamTrawler_Locale_Data::getContent($locale, 'era', array('gregorian', 'Names', '1')));
                break;

            case self::DATES :
                return $this->_toToken(SpamTrawler_Locale_Format::getDateFormat($locale), $locale);
                break;

            case self::DATE_FULL :
                return $this->_toToken(SpamTrawler_Locale_Data::getContent($locale, 'date', array('gregorian', 'full')), $locale);
                break;

            case self::DATE_LONG :
                return $this->_toToken(SpamTrawler_Locale_Data::getContent($locale, 'date', array('gregorian', 'long')), $locale);
                break;

            case self::DATE_MEDIUM :
                return $this->_toToken(SpamTrawler_Locale_Data::getContent($locale, 'date', array('gregorian', 'medium')), $locale);
                break;

            case self::DATE_SHORT :
                return $this->_toToken(SpamTrawler_Locale_Data::getContent($locale, 'date', array('gregorian', 'short')), $locale);
                break;

            case self::TIMES :
                return $this->_toToken(SpamTrawler_Locale_Format::getTimeFormat($locale), $locale);
                break;

            case self::TIME_FULL :
                return $this->_toToken(SpamTrawler_Locale_Data::getContent($locale, 'time', 'full'), $locale);
                break;

            case self::TIME_LONG :
                return $this->_toToken(SpamTrawler_Locale_Data::getContent($locale, 'time', 'long'), $locale);
                break;

            case self::TIME_MEDIUM :
                return $this->_toToken(SpamTrawler_Locale_Data::getContent($locale, 'time', 'medium'), $locale);
                break;

            case self::TIME_SHORT :
                return $this->_toToken(SpamTrawler_Locale_Data::getContent($locale, 'time', 'short'), $locale);
                break;

            case self::DATETIME :
                return $this->_toToken(SpamTrawler_Locale_Format::getDateTimeFormat($locale), $locale);
                break;

            case self::DATETIME_FULL :
                return $this->_toToken(SpamTrawler_Locale_Data::getContent($locale, 'datetime', array('gregorian', 'full')), $locale);
                break;

            case self::DATETIME_LONG :
                return $this->_toToken(SpamTrawler_Locale_Data::getContent($locale, 'datetime', array('gregorian', 'long')), $locale);
                break;

            case self::DATETIME_MEDIUM :
                return $this->_toToken(SpamTrawler_Locale_Data::getContent($locale, 'datetime', array('gregorian', 'medium')), $locale);
                break;

            case self::DATETIME_SHORT :
                return $this->_toToken(SpamTrawler_Locale_Data::getContent($locale, 'datetime', array('gregorian', 'short')), $locale);
                break;
    ...
    private function _getDigitFromName($name)
    {
        switch($name) {
            ...
            default:
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception('Month ($name) is not a known month');
        }
    }
     ...
     * For a complete list of supported datepart values look into the docu
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Date or datepart to set
     ...
     * For a complete list of supported datepart values look into the docu.
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Date or datepart to add
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Date or datepart to subtract
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Date or datepart to compare with the date object
    ...
    /**
     * Returns a new instance of SpamTrawler_Date with the selected part copied.
     ...
     * @param  string|SpamTrawler_Locale  $locale  OPTIONAL New object's locale.  No adjustments to timezone are made.
     * @return SpamTrawler_Date New clone with requested part
     */
    ...
    public function getTimezoneFromString($zone)
    {
        ...
        if ($zone instanceof SpamTrawler_Date) {
     ...
     * @return integer|string|SpamTrawler_Date  new timestamp or SpamTrawler_Date depending on calculation
     */
    private function _assign($calc, $date, $comp = 0, $dst = false)
    {
        switch ($calc) {
            case 'set' :
                if (!empty($comp)) {
                    $this->setUnixTimestamp(call_user_func(SpamTrawler_Locale_Math::$sub, $this->getUnixTimestamp(), $comp));
                }
                $this->setUnixTimestamp(call_user_func(SpamTrawler_Locale_Math::$add, $this->getUnixTimestamp(), $date));
            ...
            case 'add' :
                ...
                break;
            case 'sub' :
                $this->setUnixTimestamp(call_user_func(SpamTrawler_Locale_Math::$sub, $this->getUnixTimestamp(), $date));
                $value = $this->getUnixTimestamp();
                break;
            default :
                // cmp - compare
                return call_user_func(SpamTrawler_Locale_Math::$comp, $comp, $date);
                break;
        }
     ...
     * @param  string|integer|array|SpamTrawler_Date  $date    Date or datepart to calculate with
     * @param  string                          $part    Part of the date to calculate, if null the timestamp is used
     * @param  string|SpamTrawler_Locale              $locale  Locale for parsing input
     * @return integer|string|SpamTrawler_Date        new timestamp
     * @throws SpamTrawler_Date_Exception
     */
    private function _calculate($calc, $date, $part, $locale)
    {
        if ($date === null) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception('parameter $date must be set, null is not allowed');
        }

        if (($part !== null) && (strlen($part) !== 2) && (SpamTrawler_Locale::isLocale($part, null, false))) {
            $locale = $part;
            $part   = null;
        }
        ...
        if ($date instanceof SpamTrawler_Date) {
        ...
        if (is_array($date) === true) {
            if (empty($part) === false) {
                switch($part) {
                    ...
                    default:
                        require_once 'SpamTrawler/Date/Exception.php';
                        throw new SpamTrawler_Date_Exception("datepart for part ($part) not found in array");
                        break;
                }
            } else {
                $hours = 0;
        ...
        switch($part) {
            ...
            case self::DAY:
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("invalid date ($date) operand, day expected", 0, null, $date);
                break;

            case self::WEEKDAY_SHORT:
                $daylist = SpamTrawler_Locale_Data::getList($locale, 'day');
                ...
                // Weekday not found
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("invalid date ($date) operand, weekday expected", 0, null, $date);
                break;
            ...
            case self::DAY_SUFFIX:
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception('day suffix not supported', 0, null, $date);
                break;
            ...
            case self::WEEKDAY_NARROW:
                $daylist = SpamTrawler_Locale_Data::getList($locale, 'day', array('gregorian', 'format', 'abbreviated'));
                $weekday = (int) $this->toString(self::WEEKDAY_DIGIT, 'iso', $locale);
                $cnt = 0;
                foreach ($daylist as $key => $value) {
            ...
            case self::WEEK:
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("invalid date ($date) operand, week expected", 0, null, $date);
                break;

            // month formats
            case self::MONTH_NAME:
                $monthlist = SpamTrawler_Locale_Data::getList($locale, 'month');
                $cnt = 0;
                ...
                // Monthname not found
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("invalid date ($date) operand, month expected", 0, null, $date);
                break;
            ...
            case self::MONTH_NAME_SHORT:
                $monthlist = SpamTrawler_Locale_Data::getList($locale, 'month', array('gregorian', 'format', 'abbreviated'));
                $cnt = 0;
                foreach ($monthlist as $key => $value) {
                    if (strtoupper($value) == strtoupper($date)) {
                        $found = $key;
            ...
            case self::MONTH_DAYS:
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception('month days not supported', 0, null, $date);
                break;

            case self::MONTH_NAME_NARROW:
                $monthlist = SpamTrawler_Locale_Data::getList($locale, 'month', array('gregorian', 'stand-alone', 'narrow'));
                $cnt       = 0;
                foreach ($monthlist as $key => $value) {
                    if (strtoupper($value) === strtoupper($date)) {
                        $found = $key;
            ...
            case self::LEAPYEAR:
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception('leap year not supported', 0, null, $date);
                break;
            ...
            case self::YEAR_8601:
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("invalid date ($date) operand, year expected", 0, null, $date);
                break;
            ...
            case self::MERIDIEM:
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception('meridiem not supported', 0, null, $date);
                break;
            ...
            case self::SWATCH:
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("invalid date ($date) operand, swatchstamp expected", 0, null, $date);
                break;
            ...
            case self::HOUR_SHORT_AM:
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("invalid date ($date) operand, hour expected", 0, null, $date);
                break;
            ...
            case self::MINUTE:
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("invalid date ($date) operand, minute expected", 0, null, $date);
                break;
            ...
            case self::SECOND:
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("invalid date ($date) operand, second expected", 0, null, $date);
                break;
            ...
            case self::MILLISECOND:
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("invalid date ($date) operand, milliseconds expected", 0, null, $date);
                break;
            ...
            case self::TIMEZONE_SECS:
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception('timezone not supported', 0, null, $date);
                break;

            case self::DAYLIGHT:
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception('daylight not supported', 0, null, $date);
                break;

            case self::GMT_DIFF:
            case self::GMT_DIFF_SEP:
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception('gmtdiff not supported', 0, null, $date);
                break;
            ...
            case self::ISO_8601:
                ...
                if (empty($datematch) and empty($timematch)) {
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("unsupported ISO8601 format ($date)", 0, null, $date);
                }
                if (!empty($timematch)) {
            ...
            case self::RFC_2822:
                ...
                if (!$result) {
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("no RFC 2822 format ($date)", 0, null, $date);
                }
            ...
            case self::TIMESTAMP:
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("invalid date ($date) operand, timestamp expected", 0, null, $date);
                break;
            ...
            case self::ERA_NAME:
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception('era not supported', 0, null, $date);
                break;

            case self::DATES:
                try {
                    $parsed = SpamTrawler_Locale_Format::getDate($date, array('locale' => $locale, 'format_type' => 'iso', 'fix_date' => true));
                    ...
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception($e->getMessage(), 0, $e, $date);
                }
                break;

            case self::DATE_FULL:
                try {
                    $format = SpamTrawler_Locale_Data::getContent($locale, 'date', array('gregorian', 'full'));
                    $parsed = SpamTrawler_Locale_Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATE_LONG:
                try {
                    $format = SpamTrawler_Locale_Data::getContent($locale, 'date', array('gregorian', 'long'));
                    $parsed = SpamTrawler_Locale_Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATE_MEDIUM:
                try {
                    $format = SpamTrawler_Locale_Data::getContent($locale, 'date', array('gregorian', 'medium'));
                    $parsed = SpamTrawler_Locale_Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATE_SHORT:
                try {
                    $format = SpamTrawler_Locale_Data::getContent($locale, 'date', array('gregorian', 'short'));
                    $parsed = SpamTrawler_Locale_Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::TIMES:
                try {
                    ...
                    $parsed = SpamTrawler_Locale_Format::getTime($date, array('locale' => $locale, 'format_type' => 'iso', 'fix_date' => true));
            ...
            case self::TIME_FULL:
                try {
                    $format = SpamTrawler_Locale_Data::getContent($locale, 'time', array('gregorian', 'full'));
                    $parsed = SpamTrawler_Locale_Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
                        $year  = 1970;
            ...
            case self::TIME_LONG:
                try {
                    $format = SpamTrawler_Locale_Data::getContent($locale, 'time', array('gregorian', 'long'));
                    $parsed = SpamTrawler_Locale_Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
            ...
            case self::TIME_MEDIUM:
                try {
                    $format = SpamTrawler_Locale_Data::getContent($locale, 'time', array('gregorian', 'medium'));
                    $parsed = SpamTrawler_Locale_Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
            ...
            case self::TIME_SHORT:
                try {
                    $format = SpamTrawler_Locale_Data::getContent($locale, 'time', array('gregorian', 'short'));
                    $parsed = SpamTrawler_Locale_Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
            ...
            case self::DATETIME:
                try {
                    $parsed = SpamTrawler_Locale_Format::getDateTime($date, array('locale' => $locale, 'format_type' => 'iso', 'fix_date' => true));
                    if (($calc == 'set') || ($calc == 'cmp')) {
                        --$parsed['month'];
                        --$month;
                        --$parsed['day'];
            ...
            case self::DATETIME_FULL:
                try {
                    $format = SpamTrawler_Locale_Data::getContent($locale, 'datetime', array('gregorian', 'full'));
                    $parsed = SpamTrawler_Locale_Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATETIME_LONG:
                try {
                    $format = SpamTrawler_Locale_Data::getContent($locale, 'datetime', array('gregorian', 'long'));
                    $parsed = SpamTrawler_Locale_Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATETIME_MEDIUM:
                try {
                    $format = SpamTrawler_Locale_Data::getContent($locale, 'datetime', array('gregorian', 'medium'));
                    $parsed = SpamTrawler_Locale_Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if (($calc == 'set') || ($calc == 'cmp')) {
                        --$parsed['month'];
                        --$month;
            ...
            case self::DATETIME_SHORT:
                try {
                    $format = SpamTrawler_Locale_Data::getContent($locale, 'datetime', array('gregorian', 'short'));
                    $parsed = SpamTrawler_Locale_Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::RFC_3339:
                ...
                if (!$result) {
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("invalid date ($date) operand, ATOM format expected", 0, null, $date);
                }
            ...
            case self::COOKIE:
                ...
                if (!$result) {
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("invalid date ($date) operand, COOKIE format expected", 0, null, $date);
                }
            ...
            case self::RFC_1036:
                ...
                if (!$result) {
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("invalid date ($date) operand, RFC 822 date format expected", 0, null, $date);
                }
            ...
            case self::RFC_850:
                ...
                if (!$result) {
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("invalid date ($date) operand, RFC 850 date format expected", 0, null, $date);
                }
            ...
            case self::RFC_1123:
                ...
                if (!$result) {
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("invalid date ($date) operand, RFC 1123 date format expected", 0, null, $date);
                }
            ...
            case self::RSS:
                ...
                if (!$result) {
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("invalid date ($date) operand, RSS date format expected", 0, null, $date);
                }
            ...
            case self::W3C:
                ...
                if (!$result) {
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("invalid date ($date) operand, W3C date format expected", 0, null, $date);
                }
            ...
            default:
                if (!is_numeric($date) || !empty($part)) {
                    try {
                        if (empty($part)) {
                            $part  = SpamTrawler_Locale_Format::getDateFormat($locale) . " ";
                            $part .= SpamTrawler_Locale_Format::getTimeFormat($locale);
                        }

                        $parsed = SpamTrawler_Locale_Format::getDate($date, array('date_format' => $part, 'locale' => $locale, 'fix_date' => true, 'format_type' => 'iso'));
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Date or datepart to equal with
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Date or datepart to compare with
     * @param  string                          $part    OPTIONAL Part of the date to compare, if null the timestamp is used
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return boolean
     * @throws SpamTrawler_Date_Exception
    ...
    /**
     * Returns only the time of the date as new SpamTrawler_Date object
     * For example:
     ...
     *
     * @param  string                    $calc    Calculation to make
     * @param  string|integer|array|SpamTrawler_Date  $time    Time to calculate with, if null the actual time is taken
     * @param  string                          $format  Timeformat for parsing input
     * @param  string|SpamTrawler_Locale              $locale  Locale for parsing input
     * @return integer|SpamTrawler_Date  new time
     * @throws SpamTrawler_Date_Exception
     */
    private function _time($calc, $time, $format, $locale)
    {
        if ($time === null) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception('parameter $time must be set, null is not allowed');
        }

        if ($time instanceof SpamTrawler_Date) {
            ...
            if (is_array($time)) {
                if ((isset($time['hour']) === true) or (isset($time['minute']) === true) or
                    ...
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("no hour, minute or second given in array");
                }
            } else {
                if (self::$_options['format_type'] == 'php') {
                    $format = SpamTrawler_Locale_Format::convertPhpToIsoFormat($format);
                ...
                try {
                    ...
                    $parsed = SpamTrawler_Locale_Format::getTime($time, array('date_format' => $format, 'locale' => $locale, 'format_type' => 'iso'));
                } catch (SpamTrawler_Locale_Exception $e) {
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception($e->getMessage(), 0, $e);
                }
            }
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $time    Time to set
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $time    Time to add
     * @param  string                          $format  OPTIONAL Timeformat for parsing input
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     ...
     * For example: HH:mm:ss -> 10 -> -10 hours
     *
     * @param  string|integer|array|SpamTrawler_Date  $time    Time to sub
     * @param  string                          $format  OPTIONAL Timeformat for parsing input
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid inteface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * For example: HH:mm:ss -> 10 -> 10 hours
     *
     * @param  string|integer|array|SpamTrawler_Date  $time    Time to compare
     * @param  string                          $format  OPTIONAL Timeformat for parsing input
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws SpamTrawler_Date_Exception
     ...
     *
     * @param  string                          $calc    Calculation to make
     * @param  string|integer|array|SpamTrawler_Date  $date    Date to calculate with, if null the actual date is taken
     * @param  string                          $format  Date format for parsing
     * @param  string|SpamTrawler_Locale              $locale  Locale for parsing input
     * @return integer|SpamTrawler_Date  new date
     * @throws SpamTrawler_Date_Exception
     */
        ...
        if ($date instanceof SpamTrawler_Date) {
            ...
            if (is_array($date)) {
                if ((isset($date['year']) === true) or (isset($date['month']) === true) or
                    ...
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("no day,month or year given in array");
                }
            } else {
                ...
                try {
                    ...
                    $parsed = SpamTrawler_Locale_Format::getDate($date, array('date_format' => $format, 'locale' => $locale, 'format_type' => 'iso'));
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Date to set
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Date to add
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Date to sub
     * @param  string                          $format  OPTIONAL Date format for parsing input
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Date to compare
     * @param  string                          $format  OPTIONAL Date format for parsing input
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws SpamTrawler_Date_Exception
     ...
     *
     * @param  string|integer|SpamTrawler_Date  $date    ISO Date to set
     * @param  string|SpamTrawler_Locale        $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|SpamTrawler_Date  $date    ISO Date to add
     * @param  string|SpamTrawler_Locale        $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|SpamTrawler_Date  $date    ISO Date to sub
     * @param  string|SpamTrawler_Locale        $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|SpamTrawler_Date  $date    RFC 822 to set
     * @param  string|SpamTrawler_Locale        $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|SpamTrawler_Date  $date    RFC 822 Date to add
     * @param  string|SpamTrawler_Locale        $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|SpamTrawler_Date  $date    RFC 822 Date to sub
     * @param  string|SpamTrawler_Locale        $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
    ...
    private function _checkLocation($location)
    {
        if (!isset($location['longitude']) or !isset($location['latitude'])) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception('Location must include \'longitude\' and \'latitude\'', 0, null, $location);
        }
        if (($location['longitude'] > 180) or ($location['longitude'] < -180)) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception('Longitude must be between -180 and 180', 0, null, $location);
        }
        if (($location['latitude'] > 90) or ($location['latitude'] < -90)) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception('Latitude must be between -90 and 90', 0, null, $location);
        }
     ...
     * For a list of cities and correct locations use the class SpamTrawler_Date_Cities
     *
     ...
     *
     * @param  integer|array|SpamTrawler_Date  $year  Year to check
     * @return boolean
     */
    public static function checkLeapYear($year)
    {
        if ($year instanceof SpamTrawler_Date) {
        ...
        if (is_array($year)) {
            if (isset($year['year']) === true) {
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("no year given in array");
            }
        }

        if (!is_numeric($year)) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception("year ($year) has to be integer for checkLeapYear()", 0, null, $year);
        }
    ...
    {
        return new SpamTrawler_Date(time(), self::TIMESTAMP, $locale);
    }
     ...
     *
     * @param  string                          $calc    Calculation to make
     * @param  string|integer|array|SpamTrawler_Date  $date    Date or Part to calculate
     ...
     *
     * @param  string                    $calc    Calculation to make
     * @param  string|integer|SpamTrawler_Date  $value   Datevalue to calculate with, if null the actual value is taken
     * @param  string|SpamTrawler_Locale        $locale  Locale for parsing input
     * @return integer|SpamTrawler_Date  new date
     * @throws SpamTrawler_Date_Exception
     */
    private function _calcvalue($calc, $value, $type, $parameter, $locale)
    {
        if ($value === null) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception("parameter $type must be set, null is not allowed");
        }
        ...
        if ($value instanceof SpamTrawler_Date) {
            // extract value from object
            $value = $value->toString($parameter, 'iso', $locale);
        } else if (!is_array($value) && !is_numeric($value) && ($type != 'iso') && ($type != 'arpa')) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception("invalid $type ($value) operand", 0, null, $value);
        }
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Year to set
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Year to add
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array|SpamTrawler_Date  $date    Year to sub
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|SpamTrawler_Date  $year    Year to compare
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string                          $calc    Calculation to make
     * @param  string|integer|array|SpamTrawler_Date  $month   Month to calculate with, if null the actual month is taken
     * @param  string|SpamTrawler_Locale              $locale  Locale for parsing input
     * @return integer|SpamTrawler_Date  new time
     * @throws SpamTrawler_Date_Exception
     */
    private function _month($calc, $month, $locale)
    {
        if ($month === null) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception('parameter $month must be set, null is not allowed');
        }
        ...
        if ($month instanceof SpamTrawler_Date) {
            ...
            if (is_numeric($month)) {
                ...
                if (isset($month['month']) === true) {
                    ...
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("no month given in array");
                }
            } else {
                $monthlist  = SpamTrawler_Locale_Data::getList($locale, 'month');
                $monthlist2 = SpamTrawler_Locale_Data::getList($locale, 'month', array('gregorian', 'format', 'abbreviated'));
                ...
                if ($found == 0) {
                    require_once 'SpamTrawler/Date/Exception.php';
                    throw new SpamTrawler_Date_Exception("unknown month name ($month)", 0, null, $month);
                }
            }
        }
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array|SpamTrawler_Date  $month   Month to set
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array|SpamTrawler_Date  $month   Month to add
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array|SpamTrawler_Date  $month   Month to sub
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|SpamTrawler_Date  $month   Month to compare
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param SpamTrawler_Locale $locale OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date
     */
     ...
     *
     * @param string      $calc    Type of calculation to make
     * @param SpamTrawler_Date   $day     Day to calculate, when null the actual day is calculated
     * @param SpamTrawler_Locale $locale  Locale for parsing input
     * @return SpamTrawler_Date|integer
     */
    private function _day($calc, $day, $locale)
    {
        if ($day === null) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception('parameter $day must be set, null is not allowed');
        }
        ...
        if ($day instanceof SpamTrawler_Date) {
        ...
        if (is_numeric($day)) {
            ...
            if (isset($day['day']) === true) {
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("no day given in array");
            }
        } else {
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $month   Day to set
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * set locale.
     *
     * @param  string|integer|array|SpamTrawler_Date  $month   Day to add
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * set locale.
     *
     * @param  string|integer|array|SpamTrawler_Date  $month   Day to sub
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|SpamTrawler_Date  $day     Day to compare
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string      $calc     Type of calculation to make
     * @param  SpamTrawler_Date   $weekday  Weekday to calculate, when null the actual weekday is calculated
     * @param  SpamTrawler_Locale $locale   Locale for parsing input
     * @return SpamTrawler_Date|integer
     * @throws SpamTrawler_Date_Exception
     */
    private function _weekday($calc, $weekday, $locale)
    {
        if ($weekday === null) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception('parameter $weekday must be set, null is not allowed');
        }
        ...
        if ($weekday instanceof SpamTrawler_Date) {
        ...
        if (is_numeric($weekday)) {
            ...
            if (isset($weekday['weekday']) === true) {
                ...
                require_once 'SpamTrawler/Date/Exception.php';
                throw new SpamTrawler_Date_Exception("no weekday given in array");
            }
        } else {
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $month   Weekday to set
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * wednesday.
     *
     * @param  string|integer|array|SpamTrawler_Date  $month   Weekday to add
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * wednesday.
     *
     * @param  string|integer|array|SpamTrawler_Date  $month   Weekday to sub
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|SpamTrawler_Date  $weekday  Weekday to compare
     * @param  string|SpamTrawler_Locale              $locale   OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $day     Day of Year to set
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $day     Day of Year to add
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $day     Day of Year to sub
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|SpamTrawler_Date  $day     Day of Year to compare
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $hour    Hour to set
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $hour    Hour to add
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $hour    Hour to sub
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|SpamTrawler_Date  $hour    Hour to compare
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $minute  Minute to set
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $minute  Minute to add
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $minute  Minute to sub
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|SpamTrawler_Date  $minute  Hour to compare
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date $second Second to set
     * @param  string|SpamTrawler_Locale             $locale (Optional) Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date $second Second to add
     * @param  string|SpamTrawler_Locale             $locale (Optional) Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date $second Second to sub
     * @param  string|SpamTrawler_Locale             $locale (Optional) Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array|SpamTrawler_Date $second Second to compare
     * @param  string|SpamTrawler_Locale             $locale (Optional) Locale for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws SpamTrawler_Date_Exception
     */
    ...
    public function setFractionalPrecision($precision)
    {
        if (!intval($precision) or ($precision < 0) or ($precision > 9)) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception("precision ($precision) must be a positive integer less than 10", 0, null, $precision);
        }
     ...
     *
     * @param  integer|SpamTrawler_Date $milli     (Optional) Millisecond to set, when null the actual millisecond is set
    ...
    public function setMilliSecond($milli = null, $precision = null)
    {
        if ($milli === null) {
            ...
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception("invalid milli second ($milli) operand", 0, null, $milli);
        }
     ...
     *
     * @param  integer|SpamTrawler_Date $milli     (Optional) Millisecond to add, when null the actual millisecond is added
     ...
     *
     * @param  integer|SpamTrawler_Date $milli     (Optional) Millisecond to sub, when null the actual millisecond is subtracted
     * @param  integer           $precision (Optional) Fractional precision for the given milliseconds
     * @return SpamTrawler_Date Provides fluid interface
     */
     ...
     *
     * @param  integer|SpamTrawler_Date  $milli  OPTIONAL Millisecond to compare, when null the actual millisecond is compared
     * @param  integer            $precision  OPTIONAL Fractional precision for the given milliseconds
     * @throws SpamTrawler_Date_Exception On invalid input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     */
    public function compareMilliSecond($milli = null, $precision = null)
    {
        ...
        if ($precision === 0) {
            require_once 'SpamTrawler/Date/Exception.php';
            throw new SpamTrawler_Date_Exception('precision is 0');
        }
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $week    Week to set
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $week    Week to add
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $week    Week to sub
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return SpamTrawler_Date Provides fluid interface
     * @throws SpamTrawler_Date_Exception
     */
     ...
     *
     * @param  string|integer|array|SpamTrawler_Date  $week    Week to compare
     * @param  string|SpamTrawler_Locale              $locale  OPTIONAL Locale for parsing input
     * @return integer 0 = equal, 1 = later, -1 = earlier
     */
     ...
     *
     * @param  string|SpamTrawler_Locale $locale (Optional) Locale for parsing input
     * @throws SpamTrawler_Date_Exception When the given locale does not exist
     * @return SpamTrawler_Date Provides fluent interface
     */
     ...
     *
     * @param  string|array|SpamTrawler_Date $date   Date to parse for correctness
     * @param  string                 $format (Optional) Format for parsing the date string
     * @param  string|SpamTrawler_Locale     $locale (Optional) Locale for parsing date parts
     * @return boolean                True when all date parts are correct
     */
    public static function isDate($date, $format = null, $locale = null)
    {
        if (!is_string($date) && !is_numeric($date) && !($date instanceof SpamTrawler_Date) &&
            !is_array($date)) {
            return false;
        }
        ...
        $locale = SpamTrawler_Locale::findLocale($locale);

        if ($format === null) {
            $format = SpamTrawler_Locale_Format::getDateFormat($locale);
        ...
        if (!is_array($date)) {
            try {
                $parsed = SpamTrawler_Locale_Format::getDate($date, array('locale' => $locale,
    ...
    protected static function _getLocalizedToken($token, $locale)
    {
        switch($token) {
            ...
            case self::RFC_2822 :
                ...
                break;
            case self::DATES :
                return SpamTrawler_Locale_Data::getContent($locale, 'date');
                break;
            case self::DATE_FULL :
                return SpamTrawler_Locale_Data::getContent($locale, 'date', array('gregorian', 'full'));
                break;
            case self::DATE_LONG :
                return SpamTrawler_Locale_Data::getContent($locale, 'date', array('gregorian', 'long'));
                break;
            case self::DATE_MEDIUM :
                return SpamTrawler_Locale_Data::getContent($locale, 'date', array('gregorian', 'medium'));
                break;
            case self::DATE_SHORT :
                return SpamTrawler_Locale_Data::getContent($locale, 'date', array('gregorian', 'short'));
                break;
            case self::TIMES :
                return SpamTrawler_Locale_Data::getContent($locale, 'time');
                break;
            case self::TIME_FULL :
                return SpamTrawler_Locale_Data::getContent($locale, 'time', array('gregorian', 'full'));
                break;
            case self::TIME_LONG :
                return SpamTrawler_Locale_Data::getContent($locale, 'time', array('gregorian', 'long'));
                break;
            case self::TIME_MEDIUM :
                return SpamTrawler_Locale_Data::getContent($locale, 'time', array('gregorian', 'medium'));
                break;
            case self::TIME_SHORT :
                return SpamTrawler_Locale_Data::getContent($locale, 'time', array('gregorian', 'short'));
                break;
            case self::DATETIME :
                return SpamTrawler_Locale_Data::getContent($locale, 'datetime');
                break;
            case self::DATETIME_FULL :
                return SpamTrawler_Locale_Data::getContent($locale, 'datetime', array('gregorian', 'full'));
                break;
            case self::DATETIME_LONG :
                return SpamTrawler_Locale_Data::getContent($locale, 'datetime', array('gregorian', 'long'));
                break;
            case self::DATETIME_MEDIUM :
                return SpamTrawler_Locale_Data::getContent($locale, 'datetime', array('gregorian', 'medium'));
                break;
            case self::DATETIME_SHORT :
                return SpamTrawler_Locale_Data::getContent($locale, 'datetime', array('gregorian', 'short'));
                break;
            case self::ATOM :
            case self::RFC_3339 :
            case self::W3C :
