...
<div class="row">
    <div class="small-4 columns">
        <input type="text" id="query_urlwhitelist" class="radius" value="" placeholder="Search">
    </div>
    <div class="small-2 columns">
        <select id="column_urlwhitelist" class="radius">
            <option value="url">URL</option>
            <option value="comment">Comment</option>
        </select>
    </div>
    <div class="small-2 columns">
        <select id="operator_urlwhitelist" class="radius">
    ...
    <div class="small-2 columns">
        <input type="button" id="btnFilter_urlwhitelist" class="k-button" value="Filter">
        <input type="button" id="btnReset_urlwhitelist" class="k-button" value="Reset">
    </div>
</div>

<div id="gridURLWhitelist"></div>

        {literal}
        <script>
            $(document).ready(function () {
                var crudServiceBaseUrl = "{/literal}{$requestprotocol}://{$linkurl}Firewall/URLWhitelist/Manage{literal}",
                ...
                $("#gridURLWhitelist").kendoGrid({
                    ...
                    detailTemplate: kendo.template($("#commentTemplateURLWhitelist").html()),
                    toolbar: ["create"],
                    columns: [
                        {
                            field: "url",
                ...
                $("#btnFilter_urlwhitelist").on("click", function(e) {
                    var ds = $("#gridURLWhitelist").data("kendoGrid").dataSource;
                    ds.filter([{
                        "field": $("#column_urlwhitelist").val(),
                        "operator": $("#operator_urlwhitelist").val(),
                        "value": $("#query_urlwhitelist").val()}
                    ]);
                });

                $("#btnReset_urlwhitelist").on("click", function(e) {
                    $("#query_urlwhitelist").val('');
                    var ds = $("#gridURLWhitelist").data("kendoGrid").dataSource;
                    ds.filter([]);
                });
            });
...
<!-- popup editor template -->
    <script id="popup_editorURLWhitelist" type="text/x-kendo-template">
       <div id="errorsURLWhitelist">Shows Errors here!</div>
    </script>

        <script type="text/x-kendo-template" id="commentTemplateURLWhitelist">
            <div class="row">
                <div>Comment:</div>
