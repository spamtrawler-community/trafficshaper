...
<div class="row">
    <div class="small-4 columns">
        <input type="text" id="query_pathwhitelist" class="radius" value="" placeholder="Search">
    </div>
    <div class="small-2 columns">
        <select id="column_pathwhitelist" class="radius">
            <option value="path">Path</option>
            <option value="comment">Comment</option>
        </select>
    </div>
    <div class="small-2 columns">
        <select id="operator_pathwhitelist" class="radius">
    ...
    <div class="small-2 columns">
        <input type="button" id="btnFilter_pathwhitelist" class="k-button" value="Filter">
        <input type="button" id="btnReset_pathwhitelist" class="k-button" value="Reset">
    </div>
</div>

<div id="gridPathWhitelist"></div>

        {literal}
        <script>
            $(document).ready(function () {
                var crudServiceBaseUrl = "{/literal}{$requestprotocol}://{$linkurl}Firewall/PathWhitelist/Manage{literal}",
                ...
                $("#gridPathWhitelist").kendoGrid({
                    ...
                    detailTemplate: kendo.template($("#commentTemplatePathWhitelist").html()),
                    toolbar: ["create"],
                    columns: [
                        {
                            field: "path",
                ...
                $("#btnFilter_pathwhitelist").on("click", function(e) {
                    var ds = $("#gridPathWhitelist").data("kendoGrid").dataSource;
                    ds.filter([{
                        "field": $("#column_pathwhitelist").val(),
                        "operator": $("#operator_pathwhitelist").val(),
                        "value": $("#query_pathwhitelist").val()}
                    ]);
                });

                $("#btnReset_pathwhitelist").on("click", function(e) {
                    $("#query_pathwhitelist").val('');
                    var ds = $("#gridPathWhitelist").data("kendoGrid").dataSource;
                    ds.filter([]);
                });
            });
...
<!-- popup editor template -->
    <script id="popup_editorPathWhitelist" type="text/x-kendo-template">
       <div id="errorsPathWhitelist">Shows Errors here!</div>
    </script>

        <script type="text/x-kendo-template" id="commentTemplatePathWhitelist">
            <div class="row">
                <div>Comment:</div>
