...
class Firewall_HostnameBlacklist_Model_Manage extends Firewall_Abstract_Model_Manage
{
    ...
    public static function validateSettings($aSettings){
        ...
        if(!ctype_digit($aSettings['conf_status'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Hostname Blacklist Status Invalid!';
        }

        if(!ctype_digit($aSettings['conf_params']['allowcaptcha'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Hostname Blacklist Allow Captcha Invalid!';
        }

        /*
        if(!ctype_digit($aSettings['conf_order'])){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Hostname Blacklist Order Invalid!';
        }
        */
        ...
        if(!ctype_alnum($sBlockReason)){
            $bErrors = true;
            Firewall_Core_Controller_Manage::$aErrors[] = 'Hostname Blacklist Block Reason Invalid!';
        }
