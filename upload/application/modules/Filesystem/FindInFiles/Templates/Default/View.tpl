...
<div class="row panel">
        <div class="small-12 columns">
            <div class="small-7 columns">
                <input id="pattern" name="pattern" type="text" placeholder="Text or Pattern to search for. Regular expressions supported!" value="" class="radius">
            </div>
...
<script>
    $(function () {
        ...
        btn.click(function () {
            ...
            if(!pattern.val()){
             alertify.alert('Please enter search term or regular expression!');
            } else {
                $('#btnFindFiles').val("Search in progress...");
                $('#ResultFindFiles').html('');
                showGrid(pattern.val(), extension.val());
            }
        });
    });
</script>
    ...
    <script>
        function showGrid ( pattern, extension ) {
            var crudServiceBaseUrl = "{/literal}{$requestprotocol}://{$linkurl}Filesystem/FindInFiles/View{literal}";
                    dataSource = new kendo.data.DataSource({
                        transport: {
                            read:  {
                                url: crudServiceBaseUrl + "/find{/literal}{$urlparameterglue}{literal}pattern=" + pattern + "&" + "extension=" + extension,
                                dataType: "json",
                                type: "POST"
                            },
